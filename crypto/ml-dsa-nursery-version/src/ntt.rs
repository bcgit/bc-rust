...
pub(crate) fn ntt(r: &mut [i32]) {
    ...
    while len > 0 {
        ...
        while start < mldsa::N {
            ...
            j = start;
            while j < start + len {
                t = reduce::montgomery_reduce(zeta as i64 * r[j + len] as i64);
                r[j + len] = r[j] - t;
                r[j] += t;
                j += 1;
            }
...
pub(crate) fn inverse_ntt_to_mont(a: &mut [i32]) {
    let mut len: usize = 1;
    let mut start: usize;
    let mut j: usize;
    let mut k: usize = 256;
    ...
    while len < mldsa::N {
        ...
        while start < mldsa::N {
            ...
            while j < start + len {
                ...
                a[j] = t + a[j + len];
                a[j + len] = t - a[j + len];
                a[j + len] = reduce::montgomery_reduce(zeta as i64 * a[j + len] as i64);
                j += 1;
            }
            start = j + len;
        }
    ...
    for item in a.iter_mut().take(mldsa::N) {
        *item = reduce::montgomery_reduce(f * *item as i64);
    }
}
