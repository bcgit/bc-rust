...
use crate::{mldsa, ntt, poly_vec_l::PolyVecL, reduce, rounding, MLDSAParams, MldsaSize, D, POLY_T1PACKED_LEN, Q, Q_INV, SEED_LEN};
...
pub(crate) fn poly_add(w1: &Polynomial, w2: &Polynomial) {
    for i in 0..N {
...
pub(crate) struct Polynomial([i32; N], i32);
...
impl<PARAMS: MLDSAParams> Polynomial<PARAMS> {
    ...
    pub(crate) fn pointwise_mult(&self, w: &Self) -> Self {
        let mut w_prime = Self::new();
        for i in 0..N {
            w_prime.coeffs[i] = montgomery_reduce((self.coeffs[i] as i64) * (w.coeffs[i] as i64));
        }
        w_prime
    }
