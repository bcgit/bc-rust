...
fn the_aliases_name_the_expected_types() {
    ...
    assert_eq!(
        size_of::<Camellia_CBC_128<Encrypting, PKCS7>>(),
        size_of::<PaddedEncryptor<Cbc<Camellia_128, Encrypting, 16, 16>, PKCS7, 16, 16, 16>>()
    );
    assert_eq!(
        size_of::<Camellia_CBC_128<Decrypting, PKCS7>>(),
        size_of::<PaddedDecryptor<Cbc<Camellia_128, Decrypting, 16, 16>, PKCS7, 16, 16, 16>>()
    );
}
...
fn every_key_length_round_trips() {
    ...
    check::<16, Camellia_CBC_128<Encrypting, PKCS7>, Camellia_CBC_128<Decrypting, PKCS7>>(
        "Camellia_CBC_128",
    );
    check::<24, Camellia_CBC_192<Encrypting, PKCS7>, Camellia_CBC_192<Decrypting, PKCS7>>(
        "Camellia_CBC_192",
    );
    check::<32, Camellia_CBC_256<Encrypting, PKCS7>, Camellia_CBC_256<Decrypting, PKCS7>>(
        "Camellia_CBC_256",
    );
}
...
fn the_padding_parameter_selects_the_scheme() {
    ...
    let (_, pkcs7) =
        Camellia_CBC_128::<Encrypting, PKCS7>::encrypt(&key::<16>(), &aligned).expect("PKCS7");
    let (_, nopad) = Camellia_CBC_128::<Encrypting, NoPadding>::encrypt(&key::<16>(), &aligned)
        .expect("NoPadding");
    assert_eq!(pkcs7.len(), 32, "PKCS7 adds a whole block to aligned data");
    assert_eq!(nopad.len(), 16, "NoPadding adds nothing");

    assert!(
        Camellia_CBC_128::<Encrypting, NoPadding>::encrypt(&key::<16>(), b"hello").is_err(),
...
fn each_encryption_gets_a_fresh_iv() {
    ...
    for _ in 0..16 {
        let (iv, ct) =
            Camellia_CBC_128::<Encrypting, PKCS7>::encrypt(&key::<16>(), &plaintext).unwrap();
        assert!(seen.insert(iv), "IV repeated across encryptions");
        let back = Camellia_CBC_128::<Decrypting, PKCS7>::decrypt(&key::<16>(), &iv, &ct).unwrap();
