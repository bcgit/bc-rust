...
use bouncycastle_rsa::rsa_4096::{
    ...
    pkcs1_v1_5_verify_sha512, pss_shake256_sign, pss_shake256_verify, pss_sign_sha256,
...
fn bench_rsa_4096(c: &mut Criterion) {
    ...
    let sig_pss_shake256 = pss_shake256_sign(&sk, MSG, &mut rng).unwrap();
    ...
    group.bench_function("pss_sign_shake256", |b| {
        b.iter(|| {
            black_box(
                pss_shake256_sign(black_box(&sk), black_box(MSG), &mut DefaultRNG::default())
                    .unwrap(),
            )
        })
    });
    group.bench_function("pss_verify_shake256", |b| {
        b.iter(|| {
            pss_shake256_verify(black_box(&pk), black_box(MSG), black_box(&sig_pss_shake256))
                .unwrap()
        })
    });
