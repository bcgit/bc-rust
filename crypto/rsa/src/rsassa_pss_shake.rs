...
use crate::emsa_pss_shake::{emsa_pss_encode_shake, emsa_pss_verify_shake};
...
/// RNG-backed entry point real callers want.
...
>(
    ...
    let em = emsa_pss_encode_shake::<X, H_LEN, S_LEN, M_PRIME_LEN, DB_LEN, K_LEN>(message, salt);
...
/// `id-RSASSA-PSS-SHAKE128` and 64 for `id-RSASSA-PSS-SHAKE256`.
pub fn sign<
    X: XOF + Default,
    const H_LEN: usize,
    const S_LEN: usize,
...
>(
    ...
    sign_with_salt::<X, H_LEN, S_LEN, M_PRIME_LEN, DB_LEN, L, L2, L21, HALF, HALF2, HALF21, K_LEN>(
        sk, message, &salt,
    )
}

/// RSASSA-PSS-VERIFY (RFC 8017 §8.1.2), mirroring [`crate::rsassa_pss::verify`] but built on
/// [`emsa_pss_verify_shake`], which recovers the salt from `EM` itself.
pub fn verify<
    X: XOF + Default,
    const H_LEN: usize,
    const S_LEN: usize,
...
>(
    ...
    if emsa_pss_verify_shake::<X, H_LEN, S_LEN, M_PRIME_LEN, DB_LEN, K_LEN>(message, &em) {
        Ok(())
    } else {
