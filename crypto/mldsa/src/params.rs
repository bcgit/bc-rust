...
// todo -- why pub? Are we returning it somewhere in the public API surface?
pub trait ByteBuffer: ZeroizablePrimitive + AsRef<[u8]> + AsMut<[u8]> {}
...
// todo -- do we really need these? Surely the general test vectors will fail if there are problems here?
...
///
/// FIPS 204, Algorithm 4 (HashML-DSA.Sign) leaves the choice of PH open, so an implementation is a
/// pairing rather than a single parameter set. Everything that varies across the pairings lives
/// here, so the algorithm itself takes one type rather than a parameter set plus a hash plus a
/// digest length.
///
