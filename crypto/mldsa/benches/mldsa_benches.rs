...
use bouncycastle_mldsa::{MLDSA44PrivateKeyExpanded, MLDSA44PublicKeyExpanded, MLDSA65PrivateKeyExpanded, MLDSA65PublicKeyExpanded, MLDSA87PrivateKeyExpanded, MLDSA87PublicKeyExpanded, MLDSAPrivateKeyTrait, MLDSAPublicKeyTrait, MLDSATrait, MLDSA44, MLDSA44_SIG_LEN, MLDSA65, MLDSA65_SIG_LEN, MLDSA87, MLDSA87_SIG_LEN};
...
fn bench_mldsa_verify_with_expanded_key(c: &mut Criterion) {
    ...
    group.bench_function("ML-DSA-44", |b| {
        b.iter(|| {
            for i in 0..sigs.len() {
                let (sig, ctx) = &sigs[i];
                black_box( MLDSA44::verify_with_expanded_key(&pk_expanded, msg, Some(&ctx.to_le_bytes()), sig).unwrap() )
            }
        })
    });
    ...
    group.bench_function("ML-DSA-65", |b| {
        b.iter(|| {
            for i in 0..sigs.len() {
                let (sig, ctx) = &sigs[i];
                black_box( MLDSA65::verify_with_expanded_key(&pk_expanded, msg, Some(&ctx.to_le_bytes()), sig).unwrap() )
            }
        })
    });
    ...
    group.bench_function("ML-DSA-87", |b| {
        b.iter(|| {
            for i in 0..sigs.len() {
                let (sig, ctx) = &sigs[i];
                black_box( MLDSA87::verify_with_expanded_key(&pk_expanded, msg, Some(&ctx.to_le_bytes()), sig).unwrap() )
            }
        })
    });
...
criterion_group!(benches, 
    bench_mldsa_keygen, bench_mldsa_keygen_and_expand,
    bench_mldsa_sign, bench_mldsa_sign_with_expanded_key, bench_mldsa_sign_from_seed, 
    bench_mldsa_verify, bench_mldsa_verify_with_expanded_key);
