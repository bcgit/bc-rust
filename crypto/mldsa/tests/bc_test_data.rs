...
// commenting out because mutants copies to a /tmp dir, and so the relative links to ../bc-test-data break
#[cfg(test)]
// mod bc_test_data {
//     use std::{fs};
//     use std::path::Path;
...
//     use bouncycastle_core::traits::{Hash, SecurityStrength, Signature, SignaturePrivateKey, SignaturePublicKey};
...
//                             &sk, None, Some(&*hex::decode(&self.context).unwrap()), &ph, [0u8; 32]).unwrap();
