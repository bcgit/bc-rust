...
mod tests {
    ...
    fn ascon_xof_round_trip() {
        let direct = AsconXof128::new().hash_xof(&DUMMY_SEED[..512], 64);
        ...
        assert_eq!(by_name.hash_xof(&DUMMY_SEED[..512], 64), direct);

        let by_const = XOFFactory::new(ASCON_XOF128_NAME).unwrap();
        assert_eq!(by_const.hash_xof(&DUMMY_SEED[..512], 64), direct);
    }
