...
//     use bouncycastle_core::key_material::{KeyMaterialTrait, KeyMaterial256, KeyType};
//     use bouncycastle_core::traits::{Hash, PHSignature, SecurityStrength, Signature, SignaturePrivateKey, SignaturePublicKey};
//     use bouncycastle_mldsa_lowmemory::{HashMLDSA44_with_SHA512, HashMLDSA65_with_SHA512, HashMLDSA87_with_SHA512, MLDSA44PrivateKey, MLDSA44PublicKey, MLDSA65PrivateKey, MLDSA65PublicKey, MLDSA87PrivateKey, MLDSA87PublicKey, MLDSAPrivateKeyTrait, MLDSATrait, MLDSA44, MLDSA44_PK_LEN, MLDSA44_SK_LEN, MLDSA65, MLDSA65_PK_LEN, MLDSA65_SK_LEN, MLDSA87, MLDSA87_PK_LEN, MLDSA87_SK_LEN};
...
//                     let sk_sized: [u8; MLDSA44_SK_LEN] = hex::decode(&self.seed).unwrap().try_into().unwrap();
...
//                     assert_eq!(pk.encode(), pk_sized);
//                     let sk_sized: [u8; MLDSA65_SK_LEN] = hex::decode(&self.seed).unwrap().try_into().unwrap();
//                     assert_eq!(sk.encode(), sk_sized);
//                 },
//                 "ML-DSA-87" => {
...
//                     assert_eq!(pk.encode(), pk_sized);
//                     let sk_sized: [u8; MLDSA87_SK_LEN] = hex::decode(&self.seed).unwrap().try_into().unwrap();
//                     assert_eq!(sk.encode(), sk_sized);
//                 },
...
//     }
//
//     // commented out because the bc test vectors are missing private key seeds, so I can't use them.
//     // #[test]
//     #[allow(non_snake_case)]
//     fn ML_DSA_sigGen() {
