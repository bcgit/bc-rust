//! Known-answer tests against Google/C2SP **Wycheproof** `ARIA-CBC-PKCS5` vectors.
//!
...
const VECTOR_FILE: &str = "aria_cbc_pkcs5_test.json";
...
fn vector_file() -> Option<PathBuf> {
    ...
    println!(
        "WARNING: {VECTOR_FILE} not found (looked in {TEST_DATA_PATHS:?}); \
         Wycheproof ARIA-CBC-PKCS5 tests will be skipped"
    );
    None
}
...
fn wycheproof_cbc_pkcs5_vectors() {
    ...
    for group in doc.get("testGroups").and_then(Value::as_array).expect("testGroups") {
        ...
        for t in group.get("tests").and_then(Value::as_array).expect("tests") {
            let was_valid = match key_size {
                128 => {
                    run_case::<ARIA_CBC_128<Encrypting, PKCS7>, ARIA_CBC_128<Decrypting, PKCS7>, 16>(
                        t,
                    )
                }
                192 => {
                    run_case::<ARIA_CBC_192<Encrypting, PKCS7>, ARIA_CBC_192<Decrypting, PKCS7>, 24>(
                        t,
                    )
                }
                256 => {
                    run_case::<ARIA_CBC_256<Encrypting, PKCS7>, ARIA_CBC_256<Decrypting, PKCS7>, 32>(
                        t,
                    )
                }
    ...
    println!("Wycheproof ARIA-CBC-PKCS5: {valid} valid and {invalid} invalid cases checked");
