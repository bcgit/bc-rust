//! Type aliases for ARIA (RFC 5794, KS X 1213-1) in CTR mode (NIST SP 800-38A Sec 6.5).
//!
...
//! the ARIA values so callers never spell them out. They add nothing to the engine: the permutation
...
/// ARIA-128 in CTR mode with a 12-byte nonce. `Dir` is [`bouncycastle_modes::Encrypting`] or
...
/// let (mut enc, nonce) = ARIA_CTR_128::<Encrypting>::do_encrypt_init(&key).unwrap();
/// let mut first = [0u8; 5];
/// let mut rest = [1u8; 30];
/// enc.do_encrypt(&mut first).unwrap();
/// enc.do_encrypt(&mut rest).unwrap();
/// let mut dec = ARIA_CTR_128::<Decrypting>::do_decrypt_init(&key, &nonce).unwrap();
...
pub type ARIA_CTR_128<Dir> = Ctr<ARIA_128, Dir, 16, BLOCK_LEN, CTR_NONCE_LEN>;

/// ARIA-192 in CTR mode with a 12-byte nonce. See [`ARIA_CTR_128`].
///
/// ```
/// use bouncycastle_aria::ARIA_CTR_192;
/// use bouncycastle_core::key_material::{KeyMaterial, KeyType};
/// use bouncycastle_core::traits::{StreamCipherDecryptor, StreamCipherEncryptor};
/// use bouncycastle_modes::{Decrypting, Encrypting};
///
...
/// ```
#[allow(non_camel_case_types)]
pub type ARIA_CTR_192<Dir> = Ctr<ARIA_192, Dir, 24, BLOCK_LEN, CTR_NONCE_LEN>;

/// ARIA-256 in CTR mode with a 12-byte nonce. See [`ARIA_CTR_128`].
///
/// ```
...
/// ```
#[allow(non_camel_case_types)]
pub type ARIA_CTR_256<Dir> = Ctr<ARIA_256, Dir, 32, BLOCK_LEN, CTR_NONCE_LEN>;
