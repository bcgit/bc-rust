...
pub trait SymmetricCipher<const KEY_LEN: usize, const INIT_DATA_LEN: usize>: Algorithm {
    ...
    /// This function returns the ciphertext as a Vec<u8>, and therefore is only available when the `alloc` feature is enabled.
    /// Returns a tuple containing the initialization data and the ciphertext.
    /// This is not available in a `no_std` build without the `alloc` feature.
    fn encrypt(
