...
/// Streaming Authenticated-Encryption-with-Associated-Data interface.
///
/// `init` / `new` / `reset` are **not** on this trait. AEAD constructions
/// vary in how they want to receive their inputs — e.g. fixed-length array
/// references for compile-time-checked key/nonce sizes (`Ascon-AEAD128`),
/// or [`KeyMaterial`] wrappers for entropy-tracked keys. Forcing those
/// through a uniform generic surface would either lose type safety or
/// invite misuse. Reset semantics also vary (e.g. NIST SP 800-232 forbids
/// reusing an Ascon-AEAD128 encryption context with the same `(key, nonce)`).
///
/// Implementations expose `new` / `init` / `reset` as inherent methods. The
/// trait covers only the operations that are uniform across AEAD
/// constructions once an instance is initialised.
///
/// # AAD then data
/// Implementations expect AAD bytes (via [`AeadCipher::process_aad_byte`] /
/// [`AeadCipher::process_aad_bytes`]) to be supplied before any message
/// bytes (via [`AeadCipher::process_byte`] / [`AeadCipher::process_bytes`]).
/// Once message processing has begun, additional AAD MUST NOT be fed in.
///
/// # Tag verification
/// On decryption, [`AeadCipher::do_final`] performs the authentication
/// check and panics on failure (the trait surface does not currently carry
/// a `Result`). Implementations that need to handle auth-failure gracefully
/// SHOULD expose a fallible inherent method as well.
pub trait AeadCipher {
    /// Feed one byte of associated data.
    fn process_aad_byte(&mut self, input: u8);

    /// Feed a slice of associated data. May be called multiple times.
    fn process_aad_bytes(&mut self, in_bytes: &[u8]);

    /// Process one byte of plaintext (encryption) or ciphertext (decryption).
    /// Returns the number of bytes written to `out_bytes`.
    fn process_byte(&mut self, input: u8, out_bytes: &mut [u8]) -> usize;

    /// Process a slice of plaintext (encryption) or ciphertext (decryption).
    /// Returns the number of bytes written to `out_bytes`.
    fn process_bytes(&mut self, in_bytes: &[u8], out_bytes: &mut [u8]) -> usize;

    /// Finalize the AEAD operation: emit the trailing partial block plus
    /// the authentication tag (encryption), or verify the tag and emit the
    /// trailing partial plaintext block (decryption). Implementations
    /// SHOULD panic on failed tag verification; callers that need to
    /// handle this gracefully SHOULD use an inherent fallible variant.
    fn do_final(&mut self, out_bytes: &mut [u8]);

    /// Return a copy of the authentication tag produced by the most recent
    /// encryption finalize. Returns an empty `Vec` before encryption has
    /// been finalised.
    ///
    /// Requires heap allocation. Callers in a `no_std + no_alloc` setting
    /// SHOULD use [`AeadCipher::get_mac_out`] instead.
    fn get_mac(&self) -> Vec<u8>;

    /// Write the authentication tag produced by the most recent encryption
    /// finalize into `out` and return the number of bytes written.
    ///
    /// Heap-allocation-free twin of [`AeadCipher::get_mac`]. If `out` is
    /// larger than the tag, only the first `tag.len()` bytes are written
    /// and the rest of `out` is left untouched. If `out` is smaller than
    /// the tag, only the first `out.len()` bytes of the tag are written.
    /// Returns 0 if no tag has yet been produced (e.g. called before
    /// `do_final` on an encryption context, or on a decryption context).
    fn get_mac_out(&self, out: &mut [u8]) -> usize;

    /// Predict the number of output bytes that the next
    /// [`AeadCipher::process_bytes`] call producing `len` input bytes will
    /// emit. Used by callers to size output buffers ahead of time.
    fn get_update_output_size(&self, len: usize) -> usize;

    /// Predict the total number of output bytes (including the trailing
    /// finalize call) that processing `len` more bytes will produce.
