...
//     use bouncycastle_mlkem_lowmemory::{MLKEM1024PrivateKey, MLKEM1024PublicKey, MLKEM512PrivateKey, MLKEM512PublicKey, MLKEM768PrivateKey, MLKEM768PublicKey, MLKEMPrivateKeyTrait, MLKEMTrait, MLKEM1024, MLKEM1024_PK_LEN, MLKEM1024_SK_LEN, MLKEM512, MLKEM512_PK_LEN, MLKEM512_SK_LEN, MLKEM768, MLKEM768_PK_LEN, MLKEM768_SK_LEN};
//     use bouncycastle_mlkem_lowmemory::mlkem::{MLKEM1024_FULL_SK_LEN, MLKEM512_FULL_SK_LEN, MLKEM768_FULL_SK_LEN};
//
...
//                     let sk_sized: [u8; MLKEM512_FULL_SK_LEN] = hex::decode(&self.dk).unwrap().try_into().unwrap();
//                     assert_eq!(sk.full_sk_encode(), sk_sized);
//                 },
//                 "ML-KEM-768" => {
//                     let (pk, sk) = MLKEM768::keygen_from_seed(&seed).unwrap();
//                     let pk_sized: [u8; MLKEM768_PK_LEN] = hex::decode(&self.ek).unwrap().try_into().unwrap();
//                     assert_eq!(pk.encode(), pk_sized);
//                     let sk_sized: [u8; MLKEM768_FULL_SK_LEN] = hex::decode(&self.dk).unwrap().try_into().unwrap();
//                     assert_eq!(sk.full_sk_encode(), sk_sized);
//                 },
//                 "ML-KEM-1024" => {
...
//                     assert_eq!(pk.encode(), pk_sized);
//                     let sk_sized: [u8; MLKEM1024_FULL_SK_LEN] = hex::decode(&self.dk).unwrap().try_into().unwrap();
//                     assert_eq!(sk.full_sk_encode(), sk_sized);
//                 },
