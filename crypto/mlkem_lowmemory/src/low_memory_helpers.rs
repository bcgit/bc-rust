...
use crate::mlkem::{q, N, POLY_BYTES};
...
pub(crate) fn pack_t_hat_row<const T_PACKED_LEN: usize>(t_hat_i: &Polynomial, row: usize, t_hat_packed: &mut [u8; T_PACKED_LEN]) {
    t_hat_packed[row * POLY_BYTES .. (row + 1) * POLY_BYTES].copy_from_slice(
        &byte_encode::<12, POLY_BYTES>(&t_hat_i)
    );
}

pub(crate) fn unpack_t_hat_row<const T_PACKED_LEN: usize>(t_hat_packed: &[u8; T_PACKED_LEN], row: usize) -> Polynomial {
    byte_decode::<12, POLY_BYTES>(
        t_hat_packed[row * POLY_BYTES .. (row + 1)*POLY_BYTES].try_into().unwrap())
}

pub(crate) fn pack_s_hat_row<const k: usize>(s_hat_i: &Polynomial, row: usize, s_hat_packed: &mut [u8]) {
    debug_assert!(s_hat_packed.len() >= k*POLY_BYTES);
    
    s_hat_packed[row*POLY_BYTES .. (row+1)*POLY_BYTES].copy_from_slice(&byte_encode::<12, POLY_BYTES>(
        s_hat_i,
    ));
}
    ...
    match du {
            ...
            let mut t = [0i16; 4];
                for j in 0..N/4 {
                    ...
                    for (l, item) in t.iter_mut().enumerate() {
                        *item = (((((u_i[4 * j + l] as u32) << 10) as i32
                            + (q as i32 / 2))
                            / q as i32)
                            & 0x3FF) as i16;
                    }
        ...
        11 => {
            let mut t = [0i16; 8];
                for j in 0..N/8 {
                    for (l, item) in t.iter_mut().enumerate() {
                        *item = (((((u_i[8 * j + l] as u32) << 11) as i32
                            + (q as i32 / 2))
                            / q as i32)
                            & 0x7FF) as i16;
                    }
...
pub(crate) fn unpack_ciphertext_u_row<const du: i16, const CT_LEN: usize>(
    ...
    match du {
        10 => { // MLKEM512 and MLKEM768
            let mut t = [0i16; 4];
            for j in 0..(N/4) {
                t[0] = ((compressed_u_i[idx] as u16)
                    | (compressed_u_i[idx + 1] as u16) << 8)
                    as i16;
                t[1] = (((compressed_u_i[idx + 1] as u16) >> 2)
                    | (compressed_u_i[idx + 2] as u16) << 6)
                    as i16;
                t[2] = (((compressed_u_i[idx + 2] as u16) >> 4)
                    | (compressed_u_i[idx + 3] as u16) << 4)
                    as i16;
                t[3] = (((compressed_u_i[idx + 3] as u16) >> 6)
                    | (compressed_u_i[idx + 4] as u16) << 2)
                    as i16;
                idx += 5;
        ...
        11 => { // MLKEM1024
            let mut t = [0i16; 8];
            for j in 0..N/8 {
                t[0] = (compressed_u_i[idx] as i32
                    | ((compressed_u_i[idx + 1] as u16) as i32) << 8)
                    as i16;
                t[1] = ((compressed_u_i[idx + 1] >> 3) as i32
                    | ((compressed_u_i[idx + 2] as u16) as i32) << 5)
                    as i16;
                ...
                t[3] = ((compressed_u_i[idx + 4] >> 1) as i32
                    | ((compressed_u_i[idx + 5] as u16) as i32) << 7)
                    as i16;
                t[4] = ((compressed_u_i[idx + 5] >> 4) as i32
                    | ((compressed_u_i[idx + 6] as u16) as i32) << 4)
                    as i16;
                ...
                t[6] = ((compressed_u_i[idx + 8] >> 2) as i32
                    | ((compressed_u_i[idx + 9] as u16) as i32) << 6)
                    as i16;
...
pub(crate) fn unpack_ciphertext_v<const k: usize, const CT_LEN: usize, const du: i16, const dv: i16>(
    c: &[u8; CT_LEN]
) -> Polynomial {
    // each of the N i16's will take du bits, so a polynomial takes N * du bits, then we have k of them
