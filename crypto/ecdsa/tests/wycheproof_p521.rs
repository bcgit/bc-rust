//! Test against the project wycheproof repo: `ecdsa_secp521r1_sha512_p1363_test.json` (raw `r ||
//! s`, against [`ECDSAP521::verify`]) and `ecdsa_secp521r1_sha512_test.json` (DER `SEQUENCE { r, s
//! }`, RFC 3279 §2.2.3, against [`ECDSAP521::verify_der`]). See `wycheproof.rs`'s docs for the
...
use bouncycastle_ecdsa::keys_p521::ECDSAP521PublicKey;
...
fn ecdsa_secp521r1_sha512_p1363_test() {
    ...
    for group in doc["testGroups"].as_array().unwrap() {
        ...
        let pk = ECDSAP521PublicKey::from_bytes(&pk_bytes).ok();
        ...
        for test in group["tests"].as_array().unwrap() {
            ...
            let result = pk.as_ref().and_then(|pk| ECDSAP521::verify(pk, &msg, None, &sig).ok());
            assert_eq!(
                result.is_some(),
                expect_valid,
    ...
    println!(
        "ecdsa_secp521r1_sha512_p1363_test: {num_tests} test cases passed ({num_valid} valid, {num_invalid} invalid)."
    );
}

/// The DER counterpart of the above: `ecdsa_secp521r1_sha512_test.json`, against
...
fn ecdsa_secp521r1_sha512_der_test() {
    ...
    for group in doc["testGroups"].as_array().unwrap() {
        ...
        for test in group["tests"].as_array().unwrap() {
            ...
            let result =
                pk.as_ref().and_then(|pk| ECDSAP521::verify_der(pk, &msg, None, &sig).ok());
            assert_eq!(
                result.is_some(),
                expect_valid,
                "tcId {tc_id}: expected valid={expect_valid}, comment={:?}",
    ...
    println!(
        "ecdsa_secp521r1_sha512_der_test: {num_tests} test cases passed ({num_valid} valid, {num_invalid} invalid)."
    );
}
