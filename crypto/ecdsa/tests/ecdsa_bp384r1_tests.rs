//! Behavioural tests for [`ECDSABp384r1`], mirroring `ecdsa_p256_tests.rs`.
...
use bouncycastle_ecdsa::keys_bp384r1::{ECDSABp384r1PrivateKey, ECDSABp384r1PublicKey, keygen};
...
fn conformance_suite() {
    ...
    framework
        .test_signature::<ECDSABp384r1PublicKey, ECDSABp384r1PrivateKey, ECDSABp384r1, ECDSABp384r1, 97, 48, 96>(
            keygen,
            true,
        );
}
...
fn key_boundary_conditions() {
    TestFrameworkSignatureKeys::new()
        .test_keys::<ECDSABp384r1PublicKey, ECDSABp384r1PrivateKey, 97, 48>(keygen);
}
...
fn sign_randomized_produces_distinct_valid_signatures() {
    ...
    let sig1 = ECDSABp384r1::sign_randomized(&sk, msg, &mut DefaultRNG::default()).unwrap();
    let sig2 = ECDSABp384r1::sign_randomized(&sk, msg, &mut DefaultRNG::default()).unwrap();
    assert_ne!(sig1, sig2, "Appendix A.3.1's k comes from fresh randomness each call");

    ECDSABp384r1::verify(&pk, msg, None, &sig1).unwrap();
    ECDSABp384r1::verify(&pk, msg, None, &sig2).unwrap();
}
...
fn sign_randomized_signature_rejected_under_wrong_key() {
    ...
    let msg = b"message";
    let sig = ECDSABp384r1::sign_randomized(&sk, msg, &mut DefaultRNG::default()).unwrap();
    match ECDSABp384r1::verify(&other_pk, msg, None, &sig) {
...
fn sign_der_matches_raw_sign_and_round_trips_through_verify_der() {
    ...
    let raw = ECDSABp384r1::sign(&sk, msg, None).unwrap();
    let (der, der_len) = ECDSABp384r1::sign_der(&sk, msg, None).unwrap();
    ...
    ECDSABp384r1::verify_der(&pk, msg, None, &der[..der_len]).unwrap();

    let mut tampered = der;
    tampered[der_len - 1] ^= 0xFF;
    assert!(
        ECDSABp384r1::verify_der(&pk, msg, None, &tampered[..der_len]).is_err(),
...
fn public_key_uncompressed_and_compressed_round_trip() {
    ...
    assert_eq!(ECDSABp384r1PublicKey::from_bytes(&uncompressed).unwrap(), pk);
    ...
    assert_eq!(ECDSABp384r1PublicKey::from_bytes(&compressed).unwrap(), pk);
}

#[test]
fn private_key_encode_round_trips_exact_bytes() {
    let mut bytes = [0u8; 48];
    bytes[47] = 0x2A; // d = 42, well within [1, n-1]
    let sk = ECDSABp384r1PrivateKey::from_bytes(&bytes).unwrap();
...
fn private_key_rejects_zero() {
    match ECDSABp384r1PrivateKey::from_bytes(&[0u8; 48]) {
...
fn keygen_from_rng_is_deterministic_given_a_deterministic_rng() {
    ...
    let (pk1, sk1) = bouncycastle_ecdsa::keys_bp384r1::keygen_from_rng(&mut FixedRng).unwrap();
    let (pk2, sk2) = bouncycastle_ecdsa::keys_bp384r1::keygen_from_rng(&mut FixedRng).unwrap();
    assert_eq!(sk1, sk2);
    assert_eq!(pk1, pk2);
}

/// FIPS 186-5 Appendix A.2.1 step 3: an RNG weaker than the 192-bit security strength
/// brainpoolP384r1's order calls for (SP 800-57 Part 1 Rev. 5, Table 2) is refused rather than silently
/// used.
#[test]
fn keygen_from_rng_rejects_an_rng_below_the_required_strength() {
    ...
    assert!(matches!(
        bouncycastle_ecdsa::keys_bp384r1::keygen_from_rng(&mut WeakRng),
