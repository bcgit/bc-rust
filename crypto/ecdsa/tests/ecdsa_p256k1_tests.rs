//! Behavioural tests for [`ECDSASecp256K1`], mirroring `ecdsa_p256_tests.rs`.
...
use bouncycastle_ecdsa::keys_p256k1::{ECDSASecp256K1PrivateKey, ECDSASecp256K1PublicKey, keygen};
...
fn conformance_suite() {
    ...
    framework
        .test_signature::<ECDSASecp256K1PublicKey, ECDSASecp256K1PrivateKey, ECDSASecp256K1, ECDSASecp256K1, 65, 32, 64>(
            keygen,
            true,
        );
}
...
fn key_boundary_conditions() {
    TestFrameworkSignatureKeys::new()
        .test_keys::<ECDSASecp256K1PublicKey, ECDSASecp256K1PrivateKey, 65, 32>(keygen);
}
...
fn sign_randomized_produces_distinct_valid_signatures() {
    ...
    let sig1 = ECDSASecp256K1::sign_randomized(&sk, msg, &mut DefaultRNG::default()).unwrap();
    let sig2 = ECDSASecp256K1::sign_randomized(&sk, msg, &mut DefaultRNG::default()).unwrap();
    assert_ne!(sig1, sig2, "Appendix A.3.1's k comes from fresh randomness each call");

    ECDSASecp256K1::verify(&pk, msg, None, &sig1).unwrap();
    ECDSASecp256K1::verify(&pk, msg, None, &sig2).unwrap();
}
...
fn sign_randomized_signature_rejected_under_wrong_key() {
    ...
    let msg = b"message";
    let sig = ECDSASecp256K1::sign_randomized(&sk, msg, &mut DefaultRNG::default()).unwrap();
    match ECDSASecp256K1::verify(&other_pk, msg, None, &sig) {
...
fn sign_der_matches_raw_sign_and_round_trips_through_verify_der() {
    ...
    let raw = ECDSASecp256K1::sign(&sk, msg, None).unwrap();
    let (der, der_len) = ECDSASecp256K1::sign_der(&sk, msg, None).unwrap();
    ...
    ECDSASecp256K1::verify_der(&pk, msg, None, &der[..der_len]).unwrap();

    let mut tampered = der;
    tampered[der_len - 1] ^= 0xFF;
    assert!(
        ECDSASecp256K1::verify_der(&pk, msg, None, &tampered[..der_len]).is_err(),
...
fn public_key_uncompressed_and_compressed_round_trip() {
    ...
    assert_eq!(ECDSASecp256K1PublicKey::from_bytes(&uncompressed).unwrap(), pk);
    ...
    assert_eq!(ECDSASecp256K1PublicKey::from_bytes(&compressed).unwrap(), pk);
}
...
fn private_key_encode_round_trips_exact_bytes() {
    ...
    let sk = ECDSASecp256K1PrivateKey::from_bytes(&bytes).unwrap();
...
fn private_key_rejects_zero() {
    match ECDSASecp256K1PrivateKey::from_bytes(&[0u8; 32]) {
...
fn keygen_from_rng_is_deterministic_given_a_deterministic_rng() {
    ...
    let (pk1, sk1) = bouncycastle_ecdsa::keys_p256k1::keygen_from_rng(&mut FixedRng).unwrap();
    let (pk2, sk2) = bouncycastle_ecdsa::keys_p256k1::keygen_from_rng(&mut FixedRng).unwrap();
    assert_eq!(sk1, sk2);
    assert_eq!(pk1, pk2);
}

/// FIPS 186-5 Appendix A.2.1 step 3: an RNG weaker than the 128-bit security strength
/// secp256k1's order calls for (SP 800-57 Part 1 Rev. 5, Table 2) is refused rather than silently
/// used.
#[test]
fn keygen_from_rng_rejects_an_rng_below_the_required_strength() {
    ...
    assert!(matches!(
        bouncycastle_ecdsa::keys_p256k1::keygen_from_rng(&mut WeakRng),
...
fn private_key_rejects_values_at_or_above_the_group_order() {
    let n =
        bouncycastle_ec::p256k1_sec1::be_bytes_from_limbs(&bouncycastle_ec::p256k1_scalar::N_LIMBS);
    let mut n_plus_1 = n;
    n_plus_1[31] += 1; // n's last byte is well below 0xff, so this never carries

    for (name, bytes) in [("n", n), ("n + 1", n_plus_1), ("2^256 - 1", [0xffu8; 32])] {
        match ECDSASecp256K1PrivateKey::from_bytes(&bytes) {
            Err(SignatureError::DecodingError(_)) => {}
    ...
    let sk = ECDSASecp256K1PrivateKey::from_bytes(&n_minus_1).expect("d = n - 1 is in range");
...
fn verify_rejects_signature_of_the_wrong_length() {
    ...
    ECDSASecp256K1::verify(&pk, msg, None, &sig).expect("the untampered signature must verify");

    for extra in 1..=3 {
        let mut too_long = sig.to_vec();
        too_long.extend(core::iter::repeat_n(0xAAu8, extra));
        match ECDSASecp256K1::verify(&pk, msg, None, &too_long) {
            Err(SignatureError::SignatureVerificationFailed) => {}
    ...
    for short in 1..=3 {
        match ECDSASecp256K1::verify(&pk, msg, None, &sig[..64 - short]) {
            Err(SignatureError::SignatureVerificationFailed) => {}
...
fn verify_rejects_r_or_s_outside_1_to_n_minus_1() {
    ...
    for (name, bad) in [("0", [0u8; 32]), ("n", n), ("2^256 - 1", [0xffu8; 32])] {
        for (half, offset) in [("r", 0usize), ("s", 32usize)] {
            ...
            match ECDSASecp256K1::verify(&pk, msg, None, &tampered) {
                Err(SignatureError::SignatureVerificationFailed) => {}
...
/// One `ECDSASecp256K1` value serves both the [`Signer`] and [`SignatureVerifier`] streaming APIs, holding
...
fn sign_final_on_verify_initialized_state_errors() {
    let (pk, _) = keygen().unwrap();
    let v = ECDSASecp256K1::verify_init(&pk, None).unwrap();
    match v.sign_final() {
...
fn verify_final_on_sign_initialized_state_errors() {
    let (_, sk) = keygen().unwrap();
    let s = ECDSASecp256K1::sign_init(&sk, None).unwrap();
