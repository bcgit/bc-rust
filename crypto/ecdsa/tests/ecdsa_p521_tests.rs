//! Behavioural tests for [`ECDSAP521`], mirroring `ecdsa_p256_tests.rs`.
...
use bouncycastle_ecdsa::keys_p521::{ECDSAP521PrivateKey, ECDSAP521PublicKey, keygen};
...
fn conformance_suite() {
    ...
    framework.test_signature::<ECDSAP521PublicKey, ECDSAP521PrivateKey, ECDSAP521, ECDSAP521, 133, 66, 132>(
        keygen,
        true,
    );
}
...
fn key_boundary_conditions() {
    TestFrameworkSignatureKeys::new()
        .test_keys::<ECDSAP521PublicKey, ECDSAP521PrivateKey, 133, 66>(keygen);
}
...
fn sign_randomized_produces_distinct_valid_signatures() {
    ...
    let sig1 = ECDSAP521::sign_randomized(&sk, msg, &mut DefaultRNG::default()).unwrap();
    let sig2 = ECDSAP521::sign_randomized(&sk, msg, &mut DefaultRNG::default()).unwrap();
    assert_ne!(sig1, sig2, "Appendix A.3.1's k comes from fresh randomness each call");

    ECDSAP521::verify(&pk, msg, None, &sig1).unwrap();
    ECDSAP521::verify(&pk, msg, None, &sig2).unwrap();
}
...
fn sign_randomized_signature_rejected_under_wrong_key() {
    ...
    let msg = b"message";
    let sig = ECDSAP521::sign_randomized(&sk, msg, &mut DefaultRNG::default()).unwrap();
    match ECDSAP521::verify(&other_pk, msg, None, &sig) {
...
fn sign_der_matches_raw_sign_and_round_trips_through_verify_der() {
    ...
    let raw = ECDSAP521::sign(&sk, msg, None).unwrap();
    let (der, der_len) = ECDSAP521::sign_der(&sk, msg, None).unwrap();

    let mut r_out = [0u8; 66];
    let mut s_out = [0u8; 66];
    ...
    assert_eq!(r_out, raw[..66], "DER-decoded r must match sign's raw r");
    assert_eq!(s_out, raw[66..], "DER-decoded s must match sign's raw s");

    ECDSAP521::verify_der(&pk, msg, None, &der[..der_len]).unwrap();

    let mut tampered = der;
    tampered[der_len - 1] ^= 0xFF;
    assert!(
        ECDSAP521::verify_der(&pk, msg, None, &tampered[..der_len]).is_err(),
...
fn public_key_uncompressed_and_compressed_round_trip() {
    ...
    assert_eq!(ECDSAP521PublicKey::from_bytes(&uncompressed).unwrap(), pk);

    let compressed_tag = if uncompressed[132] & 1 == 0 { 0x02 } else { 0x03 };
    let mut compressed = [0u8; 67];
    compressed[0] = compressed_tag;
    compressed[1..].copy_from_slice(&uncompressed[1..67]);
    assert_eq!(ECDSAP521PublicKey::from_bytes(&compressed).unwrap(), pk);
}

#[test]
fn private_key_rejects_zero() {
    match ECDSAP521PrivateKey::from_bytes(&[0u8; 66]) {
...
fn private_key_encode_round_trips_exact_bytes() {
    ...
    bytes[65] = 0x2A; // d = 42
    let sk = ECDSAP521PrivateKey::from_bytes(&bytes).unwrap();
    assert_eq!(sk.encode(), bytes);

    let mut out = [0xFFu8; 66];
...
fn keygen_from_rng_is_deterministic_given_a_deterministic_rng() {
    ...
    let (pk1, sk1) = bouncycastle_ecdsa::keys_p521::keygen_from_rng(&mut FixedRng).unwrap();
    let (pk2, sk2) = bouncycastle_ecdsa::keys_p521::keygen_from_rng(&mut FixedRng).unwrap();
    assert_eq!(sk1, sk2);
    assert_eq!(pk1, pk2);
}

/// FIPS 186-5 Appendix A.2.1 step 3: an RNG weaker than the 256-bit security strength
/// P-521's order calls for (SP 800-57 Part 1 Rev. 5, Table 2) is refused rather than silently
/// used.
#[test]
fn keygen_from_rng_rejects_an_rng_below_the_required_strength() {
    ...
    assert!(matches!(
        bouncycastle_ecdsa::keys_p521::keygen_from_rng(&mut WeakRng),
...
fn private_key_rejects_values_at_or_above_the_group_order() {
    let n = bouncycastle_ec::p521_sec1::be_bytes_from_limbs(&bouncycastle_ec::p521_scalar::N_LIMBS);
    let mut n_plus_1 = n;
    n_plus_1[65] += 1; // n's last byte is well below 0xff, so this never carries

    for (name, bytes) in [("n", n), ("n + 1", n_plus_1), ("2^528 - 1", [0xffu8; 66])] {
        match ECDSAP521PrivateKey::from_bytes(&bytes) {
            Err(SignatureError::DecodingError(_)) => {}
    ...
    let mut n_minus_1 = n;
    n_minus_1[65] -= 1; // n is odd, so this never borrows
    let sk = ECDSAP521PrivateKey::from_bytes(&n_minus_1).expect("d = n - 1 is in range");
...
fn verify_rejects_signature_of_the_wrong_length() {
    ...
    ECDSAP521::verify(&pk, msg, None, &sig).expect("the untampered signature must verify");

    for extra in 1..=3 {
        let mut too_long = sig.to_vec();
        too_long.extend(core::iter::repeat_n(0xAAu8, extra));
        match ECDSAP521::verify(&pk, msg, None, &too_long) {
            Err(SignatureError::SignatureVerificationFailed) => {}
    ...
    for short in 1..=3 {
        match ECDSAP521::verify(&pk, msg, None, &sig[..132 - short]) {
            Err(SignatureError::SignatureVerificationFailed) => {}
...
fn verify_rejects_r_or_s_outside_1_to_n_minus_1() {
    ...
    let n = bouncycastle_ec::p521_sec1::be_bytes_from_limbs(&bouncycastle_ec::p521_scalar::N_LIMBS);
    for (name, bad) in [("0", [0u8; 66]), ("n", n), ("2^528 - 1", [0xffu8; 66])] {
        for (half, offset) in [("r", 0usize), ("s", 66usize)] {
            let mut tampered = sig;
            tampered[offset..offset + 66].copy_from_slice(&bad);
            match ECDSAP521::verify(&pk, msg, None, &tampered) {
                Err(SignatureError::SignatureVerificationFailed) => {}
...
/// One `ECDSAP521` value serves both the [`Signer`] and [`SignatureVerifier`] streaming APIs, holding
...
fn sign_final_on_verify_initialized_state_errors() {
    let (pk, _) = keygen().unwrap();
    let v = ECDSAP521::verify_init(&pk, None).unwrap();
    match v.sign_final() {
...
fn verify_final_on_sign_initialized_state_errors() {
    let (_, sk) = keygen().unwrap();
    let s = ECDSAP521::sign_init(&sk, None).unwrap();
    match s.verify_final(&[0u8; 132]) {
        Err(SignatureError::GenericError(_)) => {}
        other => panic!("expected GenericError, got {other:?}"),
    }
}
