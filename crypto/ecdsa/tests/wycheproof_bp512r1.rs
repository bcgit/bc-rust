//! Test against the project wycheproof repo: `ecdsa_brainpoolP512r1_sha512_p1363_test.json` (raw
//! `r || s`, against [`ECDSABp512r1::verify`]) and `ecdsa_brainpoolP512r1_sha512_test.json` (DER
//! `SEQUENCE { r, s }`, RFC 3279 §2.2.3, against [`ECDSABp512r1::verify_der`]). See
...
use bouncycastle_ecdsa::keys_bp512r1::ECDSABp512r1PublicKey;
...
#[test]
fn ecdsa_brainpool_p512r1_sha512_p1363_test() {
    let Some(contents) = get_test_data("ecdsa_brainpoolP512r1_sha512_p1363_test.json") else {
        return;
    };
    ...
    for group in doc["testGroups"].as_array().unwrap() {
        ...
        for test in group["tests"].as_array().unwrap() {
            ...
            let result = pk.as_ref().and_then(|pk| ECDSABp512r1::verify(pk, &msg, None, &sig).ok());
            assert_eq!(
                result.is_some(),
                expect_valid,
    ...
    println!(
        "ecdsa_brainpoolP512r1_sha512_p1363_test: {num_tests} test cases passed ({num_valid} valid, {num_invalid} invalid)."
    );
}

/// The DER counterpart of the above: `ecdsa_brainpoolP512r1_sha512_test.json`, against
/// [`ECDSABp512r1::verify_der`].
#[test]
fn ecdsa_brainpool_p512r1_sha512_der_test() {
    let Some(contents) = get_test_data("ecdsa_brainpoolP512r1_sha512_test.json") else {
        return;
    };
    let doc: Value = serde_json::from_str(&contents).unwrap();
    ...
    for group in doc["testGroups"].as_array().unwrap() {
        ...
        for test in group["tests"].as_array().unwrap() {
            ...
            let result =
                pk.as_ref().and_then(|pk| ECDSABp512r1::verify_der(pk, &msg, None, &sig).ok());
            assert_eq!(
                result.is_some(),
                expect_valid,
                "tcId {tc_id}: expected valid={expect_valid}, comment={:?}",
    ...
    println!(
        "ecdsa_brainpoolP512r1_sha512_der_test: {num_tests} test cases passed ({num_valid} valid, {num_invalid} invalid)."
    );
}
