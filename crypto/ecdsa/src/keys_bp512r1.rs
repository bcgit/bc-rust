//! ECDSA brainpoolP512r1 key types (FIPS 186-5 §6.2) and key-pair generation (Appendix A.2.1).
//! Identical in shape to [`crate::keys`] -- see that module's docs for the full reasoning -- with
//! brainpoolP512r1's types and SEC 1 encoding widths substituted, and [`crate::extra_bits_bp512r1`]
...
/// Encoded length of a brainpoolP512r1 private key: SEC 1 §2.3.7 integer-to-octet-string of `d`,
/// 64 bytes.
pub const SK_LEN: usize = 64;

/// Encoded length of a brainpoolP512r1 public key in the canonical uncompressed form (SEC 1
/// §2.3.3, `04 || X || Y`); [`ECDSABp512r1PublicKey::from_bytes`] also accepts the 65-byte
/// compressed form.
pub const PK_LEN: usize = 129;

/// Requested output length, in bytes, from the DRBG for FIPS 186-5 Appendix A.2.1 key generation
/// and Appendix A.3.1 randomised per-message secret generation: `576` bits (72 bytes) -- derived
/// directly from Appendix A.4.1's own bias-bound check for brainpoolP512r1's `n` (see
/// [`crate::extra_bits_bp512r1`]'s docs), not copied from any other curve's table entry.
pub(crate) const EXTRA_BITS_DRBG_OUTPUT_LEN: usize = 72;

/// An ECDSA brainpoolP512r1 private key: FIPS 186-5 §6.2's `d`, `d` in `[1, n-1]`, held in
/// [`bouncycastle_utils::secret::Secret`] via [`Bp512r1Scalar`].
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ECDSABp512r1PrivateKey(Bp512r1Scalar);

impl ECDSABp512r1PrivateKey {
...
impl SignaturePrivateKey<SK_LEN> for ECDSABp512r1PrivateKey {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self, SignatureError> {
        let array: [u8; SK_LEN] = bytes.try_into().map_err(|_| {
            SignatureError::DecodingError("ECDSA brainpoolP512r1 private key must be 64 bytes")
        })?;
        // FIPS 186-5 §6.2: d is in [1, n-1], checked on the raw big-endian value *before* any
        // reduction. `Bp512r1Scalar::from_be_bytes` reduces mod n, so checking afterwards would
        ...
        let limbs = bp512r1_sec1::limbs_from_be_bytes(&array);
        let (_, borrow) = nat::sub(&limbs, &N_LIMBS);
        if limbs == [0; 8] || borrow != 1 {
            return Err(SignatureError::DecodingError(
                "ECDSA brainpoolP512r1 private key must be in [1, n-1]",
            ));
        }
        Ok(Self(Bp512r1Scalar::from_limbs(limbs)))
    }
}

/// An ECDSA brainpoolP512r1 public key: FIPS 186-5 §6.2's `Q = [d]G`, stored as affine
/// coordinates.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ECDSABp512r1PublicKey {
    pub(crate) x: Bp512r1FieldElement,
    pub(crate) y: Bp512r1FieldElement,
}

impl SignaturePublicKey<PK_LEN> for ECDSABp512r1PublicKey {
    fn encode(&self) -> [u8; PK_LEN] {
        bp512r1_sec1::encode_uncompressed(&self.x, &self.y)
    }
    ...
    /// Accepts both SEC 1 §2.3.3 encodings (65-byte compressed, 129-byte uncompressed), fully
    /// validated per [`bp512r1_sec1::decode`]'s docs (partial validation is full validation here:
    /// RFC 5639 §3.4 gives brainpoolP512r1 cofactor `h = 1`).
    fn from_bytes(bytes: &[u8]) -> Result<Self, SignatureError> {
        let (x, y) = bp512r1_sec1::decode(bytes).ok_or(SignatureError::DecodingError(
            "invalid or out-of-range SEC 1 brainpoolP512r1 point encoding",
        ))?;
        Ok(Self { x, y })
    }
}

impl Display for ECDSABp512r1PublicKey {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "ECDSABp512r1PublicKey {{ {:x?} }}", self.encode())
    }
}

/// FIPS 186-5 Appendix A.2.1: generates an ECDSA brainpoolP512r1 key pair, sourcing the DRBG
/// output from the library's default OS-backed RNG.
pub fn keygen() -> Result<(ECDSABp512r1PublicKey, ECDSABp512r1PrivateKey), SignatureError> {
...
/// Table 2 associates with brainpoolP512r1's 512-bit order (`f = 512+` in that table's ECC column, giving
/// 256-bit security).
pub fn keygen_from_rng(
    rng: &mut dyn RNG,
) -> Result<(ECDSABp512r1PublicKey, ECDSABp512r1PrivateKey), SignatureError> {
    // FIPS 186-5 Appendix A.2.1 step 3: the DRBG must offer at least the security
    // strength brainpoolP512r1's 512-bit order calls for (SP 800-57 Part 1 Rev. 5, Table 2: 256 bits).
    ...
    Ok((ECDSABp512r1PublicKey { x, y }, ECDSABp512r1PrivateKey(d)))
}
