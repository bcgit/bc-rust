//! `ECDSABp512r1`: FIPS 186-5 §6.4.1 (signature generation, deterministic per §6.3.2/Appendix
//! A.3.3 by default) and §6.4.2 (verification), fixed to SHA-512 (the pairing wycheproof's
//! `ecdsa_brainpoolP512r1_sha512_*` vector files use). Identical in shape to [`crate::ecdsa_p256`]
//! -- see that module's docs for the full reasoning (`e = bits2int(H)` needs no truncation since
//! `len(n) == hashlen == 512`; `x_affine_of_signing_point`'s branch-free-on-a-secret-value
//! argument; both hold unchanged here) -- with brainpoolP512r1's types, [`crate::keys_bp512r1`],
//! [`crate::extra_bits_bp512r1`], and [`crate::rfc6979_bp512r1`] substituted.

use crate::extra_bits_bp512r1::reduce_wide_bits_mod_n_minus_1;
use crate::keys_bp512r1::{ECDSABp512r1PrivateKey, ECDSABp512r1PublicKey, PK_LEN, SK_LEN};
use crate::rfc6979_bp512r1;
...
/// Raw `r || s` signature length: two 64-byte field-width integers, the default encoding.
/// [`ECDSABp512r1::sign_der`]/[`ECDSABp512r1::verify_der`] offer the RFC 3279
/// §2.2.3 DER `SEQUENCE { r, s }` alternative via [`crate::der`], for interop that needs it.
pub const SIG_LEN: usize = 128;

/// Upper bound on a DER-encoded brainpoolP512r1 signature (`r`, `s` each 64 bytes); see
/// [`crate::der::max_len`].
pub const DER_SIG_MAX_LEN: usize = crate::der::max_len(64);

/// Streaming state for both `ECDSABp512r1`'s [`Signer`] and [`SignatureVerifier`] impls, mirroring
/// [`crate::ecdsa_p256::ECDSAP256`]'s shape.
pub struct ECDSABp512r1 {
    hash: SHA512,
    sk: Option<ECDSABp512r1PrivateKey>,
    pk: Option<ECDSABp512r1PublicKey>,
}

/// `x * z^-2`, branch-free -- see [`crate::ecdsa_p256`]'s module docs for why this isn't
/// [`Bp512r1JacobianPoint::to_affine`].
fn x_affine_of_signing_point(point: &Bp512r1JacobianPoint) -> Bp512r1FieldElement {
...
fn sign_with_k(
    sk: &ECDSABp512r1PrivateKey,
    e: &Bp512r1ScalarField,
    k: Bp512r1Scalar,
) -> Result<[u8; SIG_LEN], SignatureError> {
    let mut k_field = Bp512r1ScalarField::from_secret(&k);
    ...
    let r = Bp512r1ScalarField::from_limbs(x_r.to_limbs()); // step 8

    let mut d_field = Bp512r1ScalarField::from_secret(sk.scalar());
    ...
    // step 11: astronomically unlikely (r, s range over ~2^512 values each); this comparison is
    ...
    sig[..64].copy_from_slice(&bp512r1_sec1::be_bytes_from_limbs(&r.to_limbs()));
    sig[64..].copy_from_slice(&bp512r1_sec1::be_bytes_from_limbs(&s.to_limbs()));
    Ok(sig)
}

/// `e` from a 64-byte message hash: FIPS 186-5 §6.4.1 step 2 / §6.4.2 step 3, `E = H` (no
/// truncation needed -- see the module docs) then Appendix B.2.1's big-endian conversion.
fn e_from_hash(h: &[u8; 64]) -> Bp512r1ScalarField {
    Bp512r1ScalarField::from_limbs(bp512r1_sec1::limbs_from_be_bytes(h))
}
...
fn r_or_s_is_zero(r: &Bp512r1ScalarField, s: &Bp512r1ScalarField) -> bool {
    *r == Bp512r1ScalarField::ZERO || *s == Bp512r1ScalarField::ZERO
}
...
impl ECDSABp512r1 {
    ...
    pub fn sign_der(
        ...
        let len = crate::der::encode(&raw[..64], &raw[64..], &mut out);
        Ok((out, len))
    }
...
impl Signer<ECDSABp512r1PrivateKey, SK_LEN, SIG_LEN> for ECDSABp512r1 {
    ...
    fn sign_init(sk: &ECDSABp512r1PrivateKey, _ctx: Option<&[u8]>) -> Result<Self, SignatureError> {
    ...
    fn sign_final(self) -> Result<[u8; SIG_LEN], SignatureError> {
        let sk = self.sk.ok_or(SignatureError::GenericError(
            "sign_final called on a verify-initialized ECDSABp512r1; call verify_final instead",
        ))?;
        let mut h = [0u8; 64];
        self.hash.do_final_out(&mut h);
        let e = e_from_hash(&h);
        let k = rfc6979_bp512r1::generate_k(sk.scalar(), &h);
        sign_with_k(&sk, &e, k)
    }
...
impl SignatureVerifier<ECDSABp512r1PublicKey, PK_LEN, SIG_LEN> for ECDSABp512r1 {
    fn verify(
        pk: &ECDSABp512r1PublicKey,
        msg: &[u8],
        ctx: Option<&[u8]>,
    ...
    fn verify_final(self, sig: &[u8]) -> Result<(), SignatureError> {
        ...
        let r_limbs = bp512r1_sec1::limbs_from_be_bytes(&sig[..64].try_into().unwrap());
        let s_limbs = bp512r1_sec1::limbs_from_be_bytes(&sig[64..128].try_into().unwrap());
        if !in_range(&r_limbs) || !in_range(&s_limbs) {
            return Err(SignatureError::SignatureVerificationFailed);
        }
        let r = Bp512r1ScalarField::from_limbs(r_limbs);
        ...
        let s_inv = Bp512r1PublicScalar::from_limbs(s_limbs).invert_vartime(); // step 4
        let u = e.mul(&s_inv); // step 5
        let v = r.mul(&s_inv);

        let q = Bp512r1JacobianPoint::from_affine(pk.x, pk.y);
        let u_pub = Bp512r1PublicScalar::from_limbs(u.to_limbs());
        let v_pub = Bp512r1PublicScalar::from_limbs(v.to_limbs());
        ...
        let r1 = Bp512r1ScalarField::from_limbs(x_r1.to_limbs()); // steps 7-8
...
mod tests {
    ...
    fn r_or_s_is_zero_requires_only_one_to_be_zero() {
        let zero = Bp512r1ScalarField::ZERO;
        let one = Bp512r1ScalarField::ONE;
