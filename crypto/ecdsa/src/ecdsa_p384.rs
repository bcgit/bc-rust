...
use crate::keys_p384::{
    ECDSAP384PrivateKey, ECDSAP384PublicKey, EXTRA_BITS_DRBG_OUTPUT_LEN, PK_LEN, SK_LEN,
};
...
fn sign_with_k(
    ...
    let mut k_field = P384ScalarField::from_secret(&k);
    ...
    let mut d_field = P384ScalarField::from_secret(sk.scalar());
...
impl ECDSAP384 {
    ...
    /// A.3.1 step 3's `N + t` bits with `t = 64` (see [`crate::extra_bits_p384`]'s docs for why
    /// that is required here even though P-384's `n` would need none for bias alone).
...
impl SignatureVerifier<ECDSAP384PublicKey, PK_LEN, SIG_LEN> for ECDSAP384 {
    ...
    fn verify_final(self, sig: &[u8]) -> Result<(), SignatureError> {
        ...
        let s_inv = P384PublicScalar::from_limbs(s_limbs).invert_vartime();
        let u = e.mul(&s_inv);
        let v = r.mul(&s_inv);
