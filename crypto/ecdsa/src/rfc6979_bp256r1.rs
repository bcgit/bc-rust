...
//! brainpoolP256r1, fixed to SHA-256 (the pairing wycheproof's `ecdsa_brainpoolP256r1_sha256_*`
//! vector files use). Identical in shape to [`crate::rfc6979`] (P-256) -- see that module's docs
//! for the full reasoning -- since brainpoolP256r1's `n` is, like P-256's, 256 bits: `HLEN`
//! (SHA-256's output) and `qlen` are equal, so the same `int2octets`/`bits2octets`/single-HMAC-
//! round shortcuts apply. `bits2octets`'s single-conditional-subtraction reduction is valid for
//! the same reason as secp256k1's: brainpoolP256r1's `n` also has its top bit set (`n >
//! 2^255`), so `2n > 2^256` and any 256-bit input is already `< 2n` -- true regardless of how
//! close `n` is to `2^256` itself (unlike secp256k1, brainpoolP256r1's `n` is not close to a power
//! of two at all; only the top-bit-set property matters here).
//!
//! RFC 6979 has no official Appendix A.2.x-style test vectors for brainpool curves (its appendix
//! only covers the NIST curves P-192 through P-521); `rfc6979_bp256r1_vectors_tests.rs`
//! cross-checks this implementation against an independent Python implementation of the same RFC
//! instead.
...
/// brainpoolP256r1's `n` not being close to `2^256`).
fn bits2octets(h1: &[u8; HLEN]) -> [u8; HLEN] {
    let limbs = bp256r1_sec1::limbs_from_be_bytes(h1);
    ...
    bp256r1_sec1::be_bytes_from_limbs(&reduced)
}
...
fn candidate_in_range(candidate: &[u8; HLEN]) -> bool {
    let limbs = bp256r1_sec1::limbs_from_be_bytes(candidate);
    if limbs == [0, 0, 0, 0] {
        return false;
    }
...
pub fn generate_k(d: &Bp256r1Scalar, h1: &[u8; HLEN]) -> Bp256r1Scalar {
    ...
    *v = [0x01u8; HLEN];
    ...
    loop {
        ...
        if candidate_in_range(&v) {
            ...
            return Bp256r1Scalar::from_limbs(bp256r1_sec1::limbs_from_be_bytes(&v));
        }
...
mod tests {
    ...
    fn candidate_in_range_boundary_cases() {
        ...
        let n_bytes = bp256r1_sec1::be_bytes_from_limbs(&N_LIMBS);
