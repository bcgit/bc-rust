...
mod mlkem_tests {
    ...
    fn rfc9935_keygen() {
        ...
        let sk_seed_bytes: [u8; MLKEM_SEED_LEN] = hex::decode(
        ...
        let expected_sk = MLKEM512PrivateKey::from_bytes(&sk_seed_bytes).unwrap();
        let sk_bytes = expected_sk.encode();
        assert_eq!(sk_bytes.len(), sk_seed_bytes.len());
        assert_eq!(sk_bytes, sk_seed_bytes.as_slice());
        ...
        assert_eq!(derived_sk.pk().encode(), expected_pk_bytes.as_slice());

        // test outputing to the full FIPS encoding
        let full_sk_bytes = derived_sk.encode_full_sk();
        assert_eq!(full_sk_bytes.len(), MLKEM512_FULL_SK_LEN);
        assert_eq!(full_sk_bytes, expected_full_sk_bytes);
        ...
        // Decode and re-encode the sk, make sure you get the same thing
        let decoded_sk = MLKEM768PrivateKey::from_bytes(&sk_seed_bytes).unwrap();
        ...
        // test outputing to the full FIPS encoding
        let full_sk_bytes = derived_sk.encode_full_sk();
        assert_eq!(full_sk_bytes.len(), MLKEM768_FULL_SK_LEN);
        assert_eq!(full_sk_bytes, expected_full_sk_bytes);
        ...
        // Decode and re-encode the sk, make sure you get the same thing
        let decoded_sk = MLKEM1024PrivateKey::from_bytes(&sk_seed_bytes).unwrap();
        let sk_bytes = decoded_sk.encode();
        assert_eq!(sk_bytes.len(), sk_seed_bytes.len());
        assert_eq!(sk_bytes, sk_seed_bytes.as_slice());
        ...
        // test outputing to the full FIPS encoding
        let full_sk_bytes = derived_sk.encode_full_sk();
        assert_eq!(full_sk_bytes.len(), MLKEM1024_FULL_SK_LEN);
        assert_eq!(full_sk_bytes, expected_full_sk_bytes);
    ...
    fn test_mlkem() {
        let seed_bytes: [u8; MLKEM_SEED_LEN] = hex::decode(
        ...
        assert_eq!(sk.encode_full_sk(), expected_full_sk_bytes.as_slice());
        ...
        // encaps
        let (ss, ct) = MLKEM1024::encaps_internal(&pk, message);
    ...
    fn keygen_error_cases() {
        ...
        */
        ...
        let expected_pk_bytes: [u8; MLKEM512_PK_LEN] = hex::decode("").unwrap()
            .try_into().unwrap();
        let (derived_pk, derived_sk) = MLKEM512::keygen_from_seed(&seed).unwrap();
        assert_eq!(derived_sk.encode(), seed_bytes);
        assert_eq!(derived_pk.encode(), expected_pk_bytes.as_slice());
    ...
    /// Proves that `encaps_rng` is just `encaps_internal` with the message `m` sourced from the
    /// RNG: when the RNG hands back exactly the bytes that `m` would be, the two must produce the
    /// same shared secret and ciphertext.
    ///
    /// Determinism of `encaps_rng` itself is covered by the shared KEM test
    /// framework via `core_framework_tests`.)
    #[test]
    fn encaps_rng_matches_encaps_internal() {
        ...
        // ML-KEM-512
        let (pk, _sk) = MLKEM512::keygen().unwrap();
        let (ss_ref, ct_ref) = MLKEM512::encaps_internal(&pk, m);
        let mut rng = FixedSeedRNG::new(seed_bytes);
        let (ss, ct) = MLKEM512::encaps_rng(&pk, &mut rng).unwrap();
        ...
        // ML-KEM-768
        let (pk, _sk) = MLKEM768::keygen().unwrap();
        let (ss_ref, ct_ref) = MLKEM768::encaps_internal(&pk, m);
        let mut rng = FixedSeedRNG::new(seed_bytes);
        let (ss, ct) = MLKEM768::encaps_rng(&pk, &mut rng).unwrap();
        ...
        // ML-KEM-1024
        let (pk, _sk) = MLKEM1024::keygen().unwrap();
        let (ss_ref, ct_ref) = MLKEM1024::encaps_internal(&pk, m);
        let mut rng = FixedSeedRNG::new(seed_bytes);
        let (ss, ct) = MLKEM1024::encaps_rng(&pk, &mut rng).unwrap();
