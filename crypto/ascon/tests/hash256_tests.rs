...
fn hash256_embedded_kat() {
    for (msg_hex, md_hex) in HASH_KAT {
        ...
        // oversized-buffer zero-fill) is exercised here.
        TestFrameworkHash { enable_partial_byte_tests: false }
            .test_hash::<AsconHash256>(&msg, &expected);
    }
}
...
fn hash256_streaming_matches_one_shot() {
    ...
    h.do_update(&msg);
    h.do_final_out(&mut buf);
    assert_eq!(buf, expected);

    // Chunked do_update agrees for a range of chunk sizes.
    for chunk in [1usize, 7, 8, 9, 16, 33] {
        let mut hasher = AsconHash256::new();
        for piece in msg.chunks(chunk) {
            hasher.do_update(piece);
        }
        let mut got = [0u8; 32];
        hasher.do_final_out(&mut got);
        assert_eq!(got, expected, "chunked hash mismatch (chunk={chunk})");
    }

    // Byte-at-a-time do_update() agrees.
    let mut hasher = AsconHash256::new();
    for &b in &msg {
        hasher.do_update(&[b]);
    }
...
fn hash256_do_final_out_truncates_to_buffer() {
    ...
    h.do_update(&msg);
    let mut o = [0u8; 16];
    assert_eq!(h.do_final_out(&mut o), 16);
    assert_eq!(o, expected[..16]);
}
