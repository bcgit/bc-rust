...
fn xof128_embedded_kat() {
    for (msg_hex, md_hex) in XOF_KAT {
        ...
        // AsconXof128 has no absorb_last_partial_byte / squeeze_partial_byte_final support, so that
        // part of the framework is disabled; everything else (hash_xof, streaming, prefix property,
        // chunked absorb, absorb-after-squeeze) is exercised here.
        TestFrameworkXOF { enable_partial_byte_tests: false }
            .test_xof::<AsconXof128>(&msg, &expected);
    }
}
...
fn xof128_suspendable_state() {
    ...
    let mut resumed_sq = AsconXof128::from_suspended(squeezing_state).unwrap();
    let mut tail = [0u8; 35];
