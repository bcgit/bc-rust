...
impl AsconAead128 {
    ...
    pub fn do_update_aad(&mut self, input: &[u8]) {
        if input.is_empty() {
            return;
        }
        self.check_aad();
...
impl SymmetricCipher<KEY_LEN, NONCE_LEN> for AsconAead128 {
    ...
    fn encrypt_out(
        ...
        // No associated data for the plain SymmetricCipher view; the tag is appended to
        // `ciphertext`. `encrypt` itself checks that `ciphertext` is long enough.
