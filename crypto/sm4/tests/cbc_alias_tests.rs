...
fn every_key_length_round_trips() {
    ...
    check::<16, SM4_CBC<Encrypting, PKCS7>, SM4_CBC<Decrypting, PKCS7>>("SM4_CBC");
}
...
fn the_padding_parameter_selects_the_scheme() {
    ...
    let (_, pkcs7) = SM4_CBC::<Encrypting, PKCS7>::encrypt(&key::<16>(), &aligned).expect("PKCS7");
    let (_, nopad) =
        SM4_CBC::<Encrypting, NoPadding>::encrypt(&key::<16>(), &aligned).expect("NoPadding");
    assert_eq!(pkcs7.len(), 32, "PKCS7 adds a whole block to aligned data");
    assert_eq!(nopad.len(), 16, "NoPadding adds nothing");

    assert!(
        SM4_CBC::<Encrypting, NoPadding>::encrypt(&key::<16>(), b"hello").is_err(),
...
fn each_encryption_gets_a_fresh_iv() {
    ...
    for _ in 0..16 {
        let (iv, ct) = SM4_CBC::<Encrypting, PKCS7>::encrypt(&key::<16>(), &plaintext).unwrap();
        assert!(seen.insert(iv), "IV repeated across encryptions");
        let back = SM4_CBC::<Decrypting, PKCS7>::decrypt(&key::<16>(), &iv, &ct).unwrap();
