...
//! evaluate it as a Boolean circuit, four such words -- one per block, for four independent blocks
//! -- are split into 16-bit halves, eight words in all, and transposed so that plane `q[k]` holds
//! bit `k` of every one of the 16 bytes:
//!
//! ```text
...
//! `w`" does not matter to the S-box, which is applied to every byte independently; only that the
...
//! # Why 16-bit planes
//!
//! Eight `u32` planes would hold one word from each of eight blocks and double the throughput on
//! a 32- or 64-bit machine, at the cost of doubling every buffer in the round: the working state
//! and the planes. This crate takes the smaller working set -- that is what "lowmemory" means
//! here -- and processes four blocks per pass.
//!
//! # Provenance
//!
//! The three-stage masked-swap transpose is the one `bouncycastle-aes` and
//! `bouncycastle-camellia` use on `u32` words, narrowed to `u16`: it transposes within
//! each byte-lane, so the number of byte-lanes per word does not enter into it. All three are
//! translated from BearSSL `src/symcipher/aes_ct.c` (`br_aes_ct_ortho`) by Thomas Pornin, MIT
//! licensed. It is duplicated rather than shared because it is each crate's private detail and
//! none should depend on another.
