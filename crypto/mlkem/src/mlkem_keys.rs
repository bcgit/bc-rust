...
use crate::mlkem::{POLY_BYTES, H, q};
use crate::{ML_KEM_512_NAME, ML_KEM_768_NAME, ML_KEM_1024_NAME};
use crate::mlkem::{MLKEM512_k, MLKEM512_PK_LEN, MLKEM512_SK_LEN};
use crate::mlkem::{MLKEM768_k, MLKEM768_PK_LEN, MLKEM768_SK_LEN};
use crate::mlkem::{MLKEM1024_k, MLKEM1024_PK_LEN, MLKEM1024_SK_LEN};
...
pub type MLKEM512PrivateKey = MLKEMPrivateKey<MLKEM512_k, MLKEM512PublicKey, MLKEM512_SK_LEN, MLKEM512_PK_LEN>;
/// ML-KEM-768 Public Key
pub type MLKEM768PublicKey = MLKEMPublicKey<MLKEM768_k, MLKEM768_PK_LEN>;
/// ML-KEM-768 Private Key
pub type MLKEM768PrivateKey = MLKEMPrivateKey<MLKEM768_k, MLKEM768PublicKey, MLKEM768_SK_LEN, MLKEM768_PK_LEN>;
/// ML-KEM-1024 Public Key
pub type MLKEM1024PublicKey = MLKEMPublicKey<MLKEM1024_k, MLKEM1024_PK_LEN>;
/// ML-KEM-1024 Private Key
pub type MLKEM1024PrivateKey = MLKEMPrivateKey<MLKEM1024_k, MLKEM1024PublicKey, MLKEM1024_SK_LEN, MLKEM1024_PK_LEN>;
...
pub type MLKEM512PublicKeyExpanded = MLKEMPublicKeyExpanded<MLKEM512_k, MLKEM512PublicKey, MLKEM512_PK_LEN>;
/// ML-KEM-512 Private Key with a pre-expanded public matrix A for repeated decaps operations.
pub type MLKEM512PrivateKeyExpanded = MLKEMPrivateKeyExpanded<MLKEM512_k, MLKEM512PublicKey, MLKEM512PrivateKey, MLKEM512_SK_LEN, MLKEM512_PK_LEN>;
/// ML-KEM-768 Public Key with a pre-expanded public matrix A for repeated encaps operations.
pub type MLKEM768PublicKeyExpanded = MLKEMPublicKeyExpanded<MLKEM768_k, MLKEM768PublicKey, MLKEM768_PK_LEN>;
/// ML-KEM-768 Private Key with a pre-expanded public matrix A for repeated decaps operations.
pub type MLKEM768PrivateKeyExpanded = MLKEMPrivateKeyExpanded<MLKEM768_k, MLKEM768PublicKey, MLKEM768PrivateKey, MLKEM768_SK_LEN, MLKEM768_PK_LEN>;
/// ML-KEM-1024 Public Key with a pre-expanded public matrix A for repeated encaps operations.
pub type MLKEM1024PublicKeyExpanded = MLKEMPublicKeyExpanded<MLKEM1024_k, MLKEM1024PublicKey, MLKEM1024_PK_LEN>;
/// ML-KEM-1024 Private Key with a pre-expanded public matrix A for repeated decaps operations.
pub type MLKEM1024PrivateKeyExpanded = MLKEMPrivateKeyExpanded<MLKEM1024_k, MLKEM1024PublicKey,MLKEM1024PrivateKey, MLKEM1024_SK_LEN, MLKEM1024_PK_LEN>;
...
pub(crate) trait MLKEMPublicKeyInternalTrait<const k: usize, const PK_LEN: usize> : MLKEMPublicKeyTrait<k, PK_LEN> {
...
impl<const k: usize, const PK_LEN: usize> MLKEMPublicKeyInternalTrait<k, PK_LEN> for MLKEMPublicKey<k, PK_LEN> {
    fn new(t_hat: Vector<k>, rho: [u8; 32]) -> Self {
        Self { rho, t_hat }
    }

    fn t_hat(&self) -> &Vector<k> { &self.t_hat }
}
...
#[derive(Clone)]
pub struct MLKEMPublicKeyExpanded<const k: usize, PK: MLKEMPublicKeyInternalTrait<k, PK_LEN>, const PK_LEN: usize> {
    pub(crate) ek: PK,
...
impl<const k: usize, PK: MLKEMPublicKeyInternalTrait<k, PK_LEN>, const PK_LEN: usize>
PartialEq for MLKEMPublicKeyExpanded<k, PK, PK_LEN> {
    fn eq(&self, other: &Self) -> bool {
...
impl<const k: usize, PK: MLKEMPublicKeyInternalTrait<k, PK_LEN>, const PK_LEN: usize>
Eq for MLKEMPublicKeyExpanded<k, PK, PK_LEN> {}

impl<const k: usize, PK: MLKEMPublicKeyInternalTrait<k, PK_LEN>, const PK_LEN: usize>
Debug for MLKEMPublicKeyExpanded<k, PK, PK_LEN> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let alg = match k {
            2 => ML_KEM_512_NAME,
            3 => ML_KEM_768_NAME,
...
impl<const k: usize, PK: MLKEMPublicKeyInternalTrait<k, PK_LEN>, const PK_LEN: usize>
Display for MLKEMPublicKeyExpanded<k, PK, PK_LEN> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let alg = match k {
            2 => ML_KEM_512_NAME,
            3 => ML_KEM_768_NAME,
...
pub trait MLKEMPrivateKeyTrait<
    ...
    PK: MLKEMPublicKeyInternalTrait<k, PK_LEN>,
    const SK_LEN: usize,
    const PK_LEN: usize> : KEMPrivateKey<SK_LEN> {
...
pub(crate) trait MLKEMPrivateKeyInternalTrait<const k: usize, PK: MLKEMPublicKeyTrait<k, PK_LEN>, const SK_LEN: usize, const PK_LEN: usize> {
    /// Not exposing a constructor publicly because you should have to get an instance either by
    /// running a keygen, or by decoding an existing key.
    fn new(
        s_hat: Vector<k>,
...
impl<const k: usize, PK: MLKEMPublicKeyInternalTrait<k, PK_LEN>, const SK_LEN: usize, const PK_LEN: usize>
    ...
    fn sk_decode(sk: &[u8; SK_LEN]) -> Result<Self, KEMError> {
        ...
        if h_pk != ek.compute_hash() {
            return Err(KEMError::ConsistencyCheckFailed("Corrupted private key: computed hash of ek != h_ek stored in private key"));
        }
...
impl<const k: usize, PK: MLKEMPublicKeyInternalTrait<k, PK_LEN>, const SK_LEN: usize, const PK_LEN: usize>
    ...
    fn s_hat(&self) -> &Vector<k> { &self.s_hat }
...
Drop for MLKEMPrivateKey<k, PK, SK_LEN, PK_LEN>
{
    fn drop(&mut self) {
        ...
        self.z.fill(0u8);
        if self.seed_d.is_some() { self.seed_d.as_mut().unwrap().fill(0u8); }
    }
}
...
impl<
    ...
    const SK_LEN: usize,
    const PK_LEN: usize
> MLKEMPrivateKeyTrait<k, PK, SK_LEN, PK_LEN> for MLKEMPrivateKeyExpanded<k, PK, SK, SK_LEN, PK_LEN> {
    fn seed(&self) -> Option<KeyMaterial<64>> {
        self.dk.seed()
    }
