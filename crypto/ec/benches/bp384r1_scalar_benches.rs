//! Benchmarks for brainpoolP384r1's scalar side: the scalar field (arithmetic mod the
...
use bouncycastle_ec::bp384r1_scalar::{Bp384r1PublicScalar, Bp384r1Scalar, Bp384r1ScalarField};
...
fn bench_bp384r1_scalar(c: &mut Criterion) {
    let x = Bp384r1ScalarField::from_limbs(A_LIMBS);
    let y = Bp384r1ScalarField::from_limbs(B_LIMBS);

    let mut group = c.benchmark_group("bp384r1_scalar");
    ...
    let x_public = Bp384r1PublicScalar::from_limbs(A_LIMBS);
...
fn bench_bp384r1_scalar_mul(c: &mut Criterion) {
    let k = Bp384r1Scalar::from_limbs(A_LIMBS);
    let u = Bp384r1PublicScalar::from_limbs(A_LIMBS);
    let v = Bp384r1PublicScalar::from_limbs(B_LIMBS);
    ...
    let mut group = c.benchmark_group("bp384r1_scalar_mul");
...
criterion_group!(benches, bench_bp384r1_scalar, bench_bp384r1_scalar_mul);
