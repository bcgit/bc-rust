//! Benchmarks for SM2's scalar side: the scalar field (arithmetic mod the curve order `n`,
...
use bouncycastle_ec::sm2_scalar::{Sm2PublicScalar, Sm2Scalar, Sm2ScalarField};
...
fn bench_sm2_scalar(c: &mut Criterion) {
    let x = Sm2ScalarField::from_limbs(A_LIMBS);
    let y = Sm2ScalarField::from_limbs(B_LIMBS);

    let mut group = c.benchmark_group("sm2_scalar");
    ...
    let x_public = Sm2PublicScalar::from_limbs(A_LIMBS);
...
fn bench_sm2_scalar_mul(c: &mut Criterion) {
    let k = Sm2Scalar::from_limbs(A_LIMBS);
    let u = Sm2PublicScalar::from_limbs(A_LIMBS);
    let v = Sm2PublicScalar::from_limbs(B_LIMBS);
    ...
    let mut group = c.benchmark_group("sm2_scalar_mul");
...
criterion_group!(benches, bench_sm2_scalar, bench_sm2_scalar_mul);
