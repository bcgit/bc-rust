...
fn square_agrees_with_mul() {
    let mut rng = Xorshift64(0xC0FFEE0000000009);
    for _ in 0..5000 {
        let a = fe(rng.next_limbs());
...
fn zeroize_clears_the_value() {
    ...
    assert_ne!(a, P256ScalarField::ZERO, "precondition: the value starts non-zero");
    a.zeroize();
    assert_eq!(a, P256ScalarField::ZERO, "zeroize must leave the value at zero");

    // and it is idempotent, so a caller scrubbing twice on overlapping paths is harmless
    a.zeroize();
    assert_eq!(a, P256ScalarField::ZERO);
}
...
fn is_zero_distinguishes_zero_from_every_nonzero_limb_position() {
    assert!(P256ScalarField::ZERO.is_zero().to_bool());
    assert!(fe(bouncycastle_ec::p256_scalar::N_LIMBS).is_zero().to_bool(), "n reduces to 0");
    assert!(!P256ScalarField::ONE.is_zero().to_bool());
    for limb_idx in 0..4 {
...
fn known_answer_at_n_minus_1() {
    ...
    let n_minus_2 =
        fe([0xf3b9cac2fc63254f, 0xbce6faada7179e84, 0xffffffffffffffff, 0xffffffff00000000]);

    assert_eq!(n_minus_1.to_limbs(), n_minus_1_limbs, "Montgomery round trip of n-1");
    assert_eq!(P256PublicScalar::from_limbs(n_minus_1_limbs).to_limbs(), n_minus_1_limbs);
    assert_eq!(n_minus_1.add(&n_minus_1), n_minus_2, "(n-1) + (n-1) == n-2");
    assert_eq!(n_minus_1.add(&P256ScalarField::ONE), P256ScalarField::ZERO, "(n-1) + 1 == 0");
    assert_eq!(P256ScalarField::ZERO.sub(&P256ScalarField::ONE), n_minus_1, "0 - 1 == n-1");
    assert_eq!(n_minus_1.mul(&n_minus_1), P256ScalarField::ONE, "(n-1)^2 == 1");
    assert_eq!(n_minus_1.square(), P256ScalarField::ONE, "(n-1)^2 == 1, via square");
    assert_eq!(n_minus_1.negate(), P256ScalarField::ONE, "-(n-1) == 1");
    assert_eq!(n_minus_1.invert(), n_minus_1, "(n-1)^-1 == n-1");
    assert_eq!(P256ScalarField::ONE.invert(), P256ScalarField::ONE, "1^-1 == 1");
}

/// The variable-time inverse on the public scalar type must agree with the constant-time
/// [`P256ScalarField::invert`] everywhere: on the known-answer values (whose inverses are pinned above
...
fn public_scalar_invert_vartime_agrees_with_constant_time_invert() {
    let n_minus_1 = {
        let mut l = bouncycastle_ec::p256_scalar::N_LIMBS;
        l[0] -= 1;
        l
    };
    let mut one = [0u64; 4];
    ...
    for limbs in fixed {
        assert_eq!(
            bouncycastle_ec::p256_scalar::P256PublicScalar::from_limbs(limbs).invert_vartime(),
            fe(limbs).invert(),
            "limbs = {limbs:x?}"
        );
    }
    assert_eq!(
        bouncycastle_ec::p256_scalar::P256PublicScalar::from_limbs([0u64; 4]).invert_vartime(),
        P256ScalarField::ZERO,
