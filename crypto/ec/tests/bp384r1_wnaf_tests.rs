...
fn edge_case_q_is_infinity_g_or_minus_g() {
    let g = Bp384r1JacobianPoint::from_affine(
        Bp384r1FieldElement::from_limbs(bouncycastle_ec::bp384r1_domain::G_X_LIMBS),
        Bp384r1FieldElement::from_limbs(bouncycastle_ec::bp384r1_domain::G_Y_LIMBS),
    );
    let mut u = [0u64; 6];
    u[0] = 12345;
    let mut v = [0u64; 6];
    ...
    u_minus_v[0] = 5556;
    let via_comb =
        |k: [u64; 6]| comb_multiply_base_point(&Bp384r1Scalar::from_limbs(k)).to_affine();

    // [u]G + [v]infinity == [u]G
    assert_eq!(
        shamir_multiply(&scalar(u), &scalar(v), &Bp384r1JacobianPoint::INFINITY).to_affine(),
        via_comb(u)
    );
    // [u]G + [v]G == [u+v]G
