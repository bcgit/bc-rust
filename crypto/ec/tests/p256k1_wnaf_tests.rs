...
fn edge_case_q_is_infinity_g_or_minus_g() {
    let g = P256K1JacobianPoint::from_affine(
        P256K1FieldElement::from_limbs(bouncycastle_ec::p256k1_domain::G_X_LIMBS),
        P256K1FieldElement::from_limbs(bouncycastle_ec::p256k1_domain::G_Y_LIMBS),
    );
    let mut u = [0u64; 4];
    u[0] = 12345;
    let mut v = [0u64; 4];
    ...
    u_minus_v[0] = 5556;
    let via_comb = |k: [u64; 4]| comb_multiply_base_point(&P256K1Scalar::from_limbs(k)).to_affine();

    // [u]G + [v]infinity == [u]G
    assert_eq!(
        shamir_multiply(&scalar(u), &scalar(v), &P256K1JacobianPoint::INFINITY).to_affine(),
        via_comb(u)
    );
    // [u]G + [v]G == [u+v]G
