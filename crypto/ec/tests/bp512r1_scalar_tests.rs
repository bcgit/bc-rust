...
fn is_zero_distinguishes_zero_from_every_nonzero_limb_position() {
    assert!(Bp512r1ScalarField::ZERO.is_zero().to_bool());
    assert!(fe(bouncycastle_ec::bp512r1_scalar::N_LIMBS).is_zero().to_bool(), "n reduces to 0");
    assert!(!Bp512r1ScalarField::ONE.is_zero().to_bool());
    for limb_idx in 0..8 {
...
fn known_answer_at_n_minus_1() {
    ...
    assert_eq!(Bp512r1PublicScalar::from_limbs(n_minus_1_limbs).to_limbs(), n_minus_1_limbs);
    assert_eq!(n_minus_1.add(&n_minus_1), n_minus_2, "(n-1) + (n-1) == n-2");
    assert_eq!(n_minus_1.add(&Bp512r1ScalarField::ONE), Bp512r1ScalarField::ZERO, "(n-1) + 1 == 0");
    assert_eq!(Bp512r1ScalarField::ZERO.sub(&Bp512r1ScalarField::ONE), n_minus_1, "0 - 1 == n-1");
    assert_eq!(n_minus_1.mul(&n_minus_1), Bp512r1ScalarField::ONE, "(n-1)^2 == 1");
    assert_eq!(n_minus_1.square(), Bp512r1ScalarField::ONE, "(n-1)^2 == 1, via square");
    assert_eq!(n_minus_1.negate(), Bp512r1ScalarField::ONE, "-(n-1) == 1");
    assert_eq!(n_minus_1.invert(), n_minus_1, "(n-1)^-1 == n-1");
    assert_eq!(Bp512r1ScalarField::ONE.invert(), Bp512r1ScalarField::ONE, "1^-1 == 1");
}

/// The variable-time inverse on the public scalar type must agree with the constant-time
/// [`Bp512r1ScalarField::invert`] everywhere: on the known-answer values (whose inverses are pinned above
...
fn public_scalar_invert_vartime_agrees_with_constant_time_invert() {
    let n_minus_1 = {
        let mut l = bouncycastle_ec::bp512r1_scalar::N_LIMBS;
        l[0] -= 1;
        l
    };
    let mut one = [0u64; 8];
    ...
    let mut low_limb_one_a = [0u64; 8];
    low_limb_one_a[0] = 1;
    low_limb_one_a[1] = 1;
    let mut low_limb_one_b = [0u64; 8];
    low_limb_one_b[0] = 1;
    low_limb_one_b[7] = 1;
    let mut low_limb_one_c = [0u64; 8];
    low_limb_one_c[0] = 1;
    low_limb_one_c[1] = u64::MAX;
    let mut fixed: Vec<[u64; 8]> =
        vec![[0u64; 8], one, n_minus_1, low_limb_one_a, low_limb_one_b, low_limb_one_c];
    fixed.extend([VALS_0, VALS_1, VALS_2]);
    for limbs in fixed {
        assert_eq!(
            bouncycastle_ec::bp512r1_scalar::Bp512r1PublicScalar::from_limbs(limbs)
                .invert_vartime(),
            fe(limbs).invert(),
            "limbs = {limbs:x?}"
        );
    ...
    assert_eq!(
        bouncycastle_ec::bp512r1_scalar::Bp512r1PublicScalar::from_limbs([0u64; 8])
            .invert_vartime(),
        Bp512r1ScalarField::ZERO,
        "0^-1 == 0 by convention, matching invert"
    );

    let mut rng = Xorshift64(0x1AC0_B5EC_0000_0008);
    for i in 0..500 {
