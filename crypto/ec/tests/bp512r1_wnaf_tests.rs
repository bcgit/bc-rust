...
fn edge_case_q_is_infinity_g_or_minus_g() {
    let g = Bp512r1JacobianPoint::from_affine(
        Bp512r1FieldElement::from_limbs(bouncycastle_ec::bp512r1_domain::G_X_LIMBS),
        Bp512r1FieldElement::from_limbs(bouncycastle_ec::bp512r1_domain::G_Y_LIMBS),
    );
    let mut u = [0u64; 8];
    u[0] = 12345;
    let mut v = [0u64; 8];
    v[0] = 6789;
    let mut u_plus_v = [0u64; 8];
    u_plus_v[0] = 19134;
    let mut u_minus_v = [0u64; 8];
    u_minus_v[0] = 5556;
    let via_comb =
        |k: [u64; 8]| comb_multiply_base_point(&Bp512r1Scalar::from_limbs(k)).to_affine();

    // [u]G + [v]infinity == [u]G
    assert_eq!(
        shamir_multiply(&scalar(u), &scalar(v), &Bp512r1JacobianPoint::INFINITY).to_affine(),
        via_comb(u)
    );
    // [u]G + [v]G == [u+v]G
    ...
    assert_eq!(shamir_multiply(&scalar([0u64; 8]), &scalar(v), &g).to_affine(), via_comb(v));
}
