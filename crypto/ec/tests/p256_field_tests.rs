...
fn is_zero_distinguishes_zero_from_every_nonzero_limb_position() {
    assert!(P256FieldElement::ZERO.is_zero().to_bool());
    assert!(fe(bouncycastle_ec::p256::P_LIMBS).is_zero().to_bool(), "p reduces to 0");
    assert!(!P256FieldElement::ONE.is_zero().to_bool());
    for limb_idx in 0..4 {
...
fn known_answer_add_carry_and_sub_borrow_at_the_extremes() {
    let p_minus_1 =
        fe([0xfffffffffffffffe, 0x00000000ffffffff, 0x0000000000000000, 0xffffffff00000001]);
    let p_minus_2 =
        fe([0xfffffffffffffffd, 0x00000000ffffffff, 0x0000000000000000, 0xffffffff00000001]);
    ...
    assert_eq!(p_minus_1.add(&P256FieldElement::ONE), P256FieldElement::ZERO, "(p-1) + 1 == 0");
    assert_eq!(P256FieldElement::ZERO.sub(&P256FieldElement::ONE), p_minus_1, "0 - 1 == p-1");
    assert_eq!(P256FieldElement::ONE.sub(&p_minus_1), two, "1 - (p-1) == 2");
    assert_eq!(p_minus_1.negate(), P256FieldElement::ONE, "-(p-1) == 1");
    assert_eq!(P256FieldElement::ONE.invert(), P256FieldElement::ONE, "1^-1 == 1");
