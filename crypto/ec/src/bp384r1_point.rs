...
/// The curve coefficient `A` in the field's Montgomery representation (`A * 2^384 mod p`),
/// computed in Python from RFC 5639 §3.4's `A` and pinned against
/// [`Bp384r1FieldElement::from_limbs`] by this module's tests. [`Bp384r1JacobianPoint::double`]
...
const A_MONTGOMERY_LIMBS: [u64; 6] = [
    0xdb26b895466c3c99, 0x75d7f3fef157b07b, 0x936771b9d7f10db4, 0xe7ffe9e535529374,
    0x400a8fdf42b00c60, 0x7c338021a2e8c0d1,
];
...
impl Bp384r1JacobianPoint {
    ...
    pub fn double(&self) -> Self {
        let a = Bp384r1FieldElement::from_internal_limbs(A_MONTGOMERY_LIMBS);
    ...
    pub(crate) fn add_affine(
        &self,
        x2: &Bp384r1FieldElement,
        y2: &Bp384r1FieldElement,
        ...
        let is_opposite_point = h_is_zero & !r_is_zero;
        let doubled = self.double();
        let other = Self { x: *x2, y: *y2, z: Bp384r1FieldElement::ONE };
    ...
    ///
    /// Restricted to `pub(crate)` and used only by [`crate::bp384r1_wnaf`], whose scalars are
    /// [`crate::bp384r1_scalar::Bp384r1PublicScalar`] and whose points are a signer's public key
...
fn select_point(
    ...
    Bp384r1JacobianPoint {
        x: Bp384r1FieldElement::from_internal_limbs(select_limbs(
            cond,
            &a.x.internal_limbs(),
            &b.x.internal_limbs(),
        )),
        y: Bp384r1FieldElement::from_internal_limbs(select_limbs(
            cond,
            &a.y.internal_limbs(),
            &b.y.internal_limbs(),
        )),
        z: Bp384r1FieldElement::from_internal_limbs(select_limbs(
            cond,
            &a.z.internal_limbs(),
            &b.z.internal_limbs(),
        )),
...
// it on a secret point would undo the constant-time discipline [`Bp384r1JacobianPoint::add`] exists
...
mod tests {
    ...
    fn a_montgomery_constant_is_a_in_montgomery_form() {
        assert_eq!(
            Bp384r1FieldElement::from_internal_limbs(A_MONTGOMERY_LIMBS),
            Bp384r1FieldElement::from_limbs(A_LIMBS)
        );
    }
    ...
    fn add_affine_agrees_with_add_on_every_case() {
        ...
        let z = Bp384r1FieldElement::from_limbs(G_Y_LIMBS);
        let z2 = z.mul(&z);
        let scaled_g = Bp384r1JacobianPoint { x: g.x.mul(&z2), y: g.y.mul(&z2.mul(&z)), z };
        let selves = [g, g.double(), g.negate(), Bp384r1JacobianPoint::INFINITY, scaled_g];
    ...
    fn add_vartime_affine_agrees_with_add_on_every_case() {
        ...
        let z2 = z.mul(&z);
        let scaled_g = Bp384r1JacobianPoint { x: g.x.mul(&z2), y: g.y.mul(&z2.mul(&z)), z };
        for a in [g, g.double(), g.negate(), Bp384r1JacobianPoint::INFINITY, scaled_g] {
    ...
    fn add_vartime_agrees_with_add_on_every_case() {
        ...
        let points = [g, g.double(), g.negate(), Bp384r1JacobianPoint::INFINITY];
        for a in points {
            for b in points {
                assert_eq!(
