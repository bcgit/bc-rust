//! The brainpoolP256r1 scalar field, arithmetic mod the curve order `n` (RFC 5639 §3.4). Like
//! [`crate::bp256r1`], `n` here has no special algebraic structure, so this is built on
...
/// `n`, the order of the brainpoolP256r1 base point `G`, little-endian `u64` limbs (RFC 5639
/// §3.4).
pub const N_LIMBS: [u64; 4] =
    [0x901e0e82974856a7, 0x8c397aa3b561a6f7, 0x3e660a909d838d71, 0xa9fb57dba1eea9bc];

/// `-n^-1 mod 2^64`, the Montgomery reduction constant.
const N_PRIME: u64 = 0xfbffbebdcbb40ee9;

/// `R^2 mod n` (`R = 2^256`), used to bring a plain value into Montgomery form.
const R_SQUARED_LIMBS: [u64; 4] =
    [0xe1d8d8de3312fca6, 0xf35d176a1134e4a0, 0x9b7f25e76c815cb0, 0x0b25f1b9c3236762];

/// `R mod n`, also the Montgomery representation of `1`.
const R_MOD_N_LIMBS: [u64; 4] =
    [0x6fe1f17d68b7a959, 0x73c6855c4a9e5908, 0xc199f56f627c728e, 0x5604a8245e115643];

/// `n - 2`, little-endian `u64` limbs -- the public exponent [`Bp256r1ScalarField::invert`]
/// raises its base to.
const N_MINUS_2_LIMBS: [u64; 4] =
    [0x901e0e82974856a5, 0x8c397aa3b561a6f7, 0x3e660a909d838d71, 0xa9fb57dba1eea9bc];

/// An element of the brainpoolP256r1 scalar field, i.e. an integer mod `n`. Stored internally in
/// Montgomery form; see the module docs.
#[derive(Clone, Copy, Debug)]
pub struct Bp256r1ScalarField([u64; 4]);

impl Bp256r1ScalarField {
    ...
    /// value), reducing once if `limbs >= n` (valid since `limbs < 2^256 < 2n`), then converting
    /// to Montgomery form.
    ...
    pub fn square(&self) -> Self {
        let t = montgomery::widening_square::<4, 8>(&self.0);
        Self(Self::finish_redc(&t))
    }
    ...
    pub fn from_secret(secret: &Bp256r1Scalar) -> Self {
...
impl PartialEq for Bp256r1ScalarField {
...
impl Eq for Bp256r1ScalarField {}
...
pub struct Bp256r1Scalar(Secret<[u64; 4]>);
...
impl Bp256r1Scalar {
    ...
    pub fn from_be_bytes(bytes: &[u8; 32]) -> Self {
        Self::from_limbs(crate::bp256r1_sec1::limbs_from_be_bytes(bytes))
    }

    /// Encodes to SEC 1 §2.3.7 big-endian octets.
    pub fn to_be_bytes(&self) -> [u8; 32] {
        crate::bp256r1_sec1::be_bytes_from_limbs(&self.0)
    }
...
pub struct Bp256r1PublicScalar([u64; 4]);

impl Bp256r1PublicScalar {
    ...
    /// `self^-1 mod n` in **variable time**, as a [`Bp256r1ScalarField`] ready for the arithmetic
    ...
    /// and not on [`Bp256r1ScalarField`], whose `invert` is the constant-time one. See
    /// [`crate::inverse_vartime`].
    pub fn invert_vartime(&self) -> Bp256r1ScalarField {
        Bp256r1ScalarField::from_limbs(crate::inverse_vartime::mod_inverse(&self.0, &N_LIMBS))
    }
}
