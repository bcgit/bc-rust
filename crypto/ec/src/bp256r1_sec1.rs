...
//! string encoding for brainpoolP256r1. Identical in shape to [`crate::p256_sec1`] -- see that
...
//! gives brainpoolP256r1 cofactor `h = 1`, and the compressed-decode square-root derivation) --
//! with the curve equation `y^2 = x^3 + Ax + B` (a general `A`, per [`crate::bp256r1_point`],
//! instead of `a = -3`) and brainpoolP256r1's own constants (also `p ≡ 3 (mod 4)`, verified the
...
use crate::bp256r1::{Bp256r1FieldElement, P_LIMBS};
use crate::bp256r1_domain::{A_LIMBS, B_LIMBS};
...
const SQRT_EXPONENT: [u64; 4] =
    [0x0804d20747db94de, 0x9b8efd88f549880a, 0x0f9982a42760e35c, 0x2a7ed5f6e87baa6f];
...
/// `y^2 == x^3 + Ax + B`.
fn is_on_curve(x: &Bp256r1FieldElement, y: &Bp256r1FieldElement) -> bool {
    let lhs = y.mul(y);
    let x_cubed = x.mul(x).mul(x);
    let ax = Bp256r1FieldElement::from_limbs(A_LIMBS).mul(x);
    let rhs = x_cubed.add(&ax).add(&Bp256r1FieldElement::from_limbs(B_LIMBS));
    lhs == rhs
}
...
fn pow_public(base: &Bp256r1FieldElement, exponent_limbs: &[u64; 4]) -> Bp256r1FieldElement {
    let mut result = Bp256r1FieldElement::ONE;
...
pub fn encode_uncompressed(x: &Bp256r1FieldElement, y: &Bp256r1FieldElement) -> [u8; 65] {
    let mut out = [0u8; 65];
    out[0] = 0x04;
...
pub fn encode_compressed(x: &Bp256r1FieldElement, y: &Bp256r1FieldElement) -> [u8; 33] {
    let mut out = [0u8; 33];
...
pub fn decode(bytes: &[u8]) -> Option<(Bp256r1FieldElement, Bp256r1FieldElement)> {
    match bytes.len() {
        33 => {
            ...
            let x = Bp256r1FieldElement::from_limbs(x_limbs);
            let x_cubed = x.mul(&x).mul(&x);
            let ax = Bp256r1FieldElement::from_limbs(A_LIMBS).mul(&x);
            let alpha = x_cubed.add(&ax).add(&Bp256r1FieldElement::from_limbs(B_LIMBS));
        ...
        65 => {
            ...
            let x = Bp256r1FieldElement::from_limbs(x_limbs);
            let y = Bp256r1FieldElement::from_limbs(y_limbs);
            if !is_on_curve(&x, &y) {
                return None;
            }
            Some((x, y))
...
pub fn decode_point(bytes: &[u8]) -> Option<Bp256r1JacobianPoint> {
    let (x, y) = decode(bytes)?;
    Some(Bp256r1JacobianPoint::from_affine(x, y))
}
