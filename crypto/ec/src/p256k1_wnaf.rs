//! Interleaved windowed-NAF (wNAF) Shamir's-trick multiplication: `[u]G + [v]Q`, for secp256k1
//! ECDSA verification. Identical algorithm to [`crate::p256_wnaf`] -- see that module's docs for
//! the full derivation and verification methodology -- with the limb count swapped for
//! secp256k1's (`WNAF_LEN = 257`, one more than `n`'s 256-bit length, same margin as P-256's own
//! `257 = 256 + 1`).
//!
//! `G`'s odd multiples are a compile-time affine table (`p256k1_wnaf_table`, width 7) added by mixed
...
use crate::p256k1_scalar::P256K1PublicScalar;
use crate::p256k1_wnaf_table::{G_ODD_MULTIPLES_X, G_ODD_MULTIPLES_Y};
...
pub fn shamir_multiply(
    u: &P256K1PublicScalar,
    v: &P256K1PublicScalar,
    q: &P256K1JacobianPoint,
) -> P256K1JacobianPoint {
...
/// holds `(2i+1)*p`.
fn odd_multiples(p: &P256K1JacobianPoint) -> [P256K1JacobianPoint; ODD_MULTIPLE_COUNT] {
...
/// digit `0`.
fn lookup_signed(
    table: &[P256K1JacobianPoint; ODD_MULTIPLE_COUNT],
    digit: i8,
) -> Option<P256K1JacobianPoint> {
...
fn lookup_signed_g(digit: i8) -> Option<(P256K1FieldElement, P256K1FieldElement)> {
    if digit == 0 {
        return None;
    }
    let index = (digit.unsigned_abs() as usize - 1) / 2;
    let x = P256K1FieldElement::from_limbs(G_ODD_MULTIPLES_X[index]);
    let y = P256K1FieldElement::from_limbs(G_ODD_MULTIPLES_Y[index]);
