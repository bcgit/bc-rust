...
use crate::sm2_wnaf_table::{G_ODD_MULTIPLES_X, G_ODD_MULTIPLES_Y};
...
fn lookup_signed_g(digit: i8) -> Option<(Sm2FieldElement, Sm2FieldElement)> {
    if digit == 0 {
        return None;
    }
    let index = (digit.unsigned_abs() as usize - 1) / 2;
    let x = Sm2FieldElement::from_limbs(G_ODD_MULTIPLES_X[index]);
    let y = Sm2FieldElement::from_limbs(G_ODD_MULTIPLES_Y[index]);
