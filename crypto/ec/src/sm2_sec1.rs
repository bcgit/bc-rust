//! SEC 1 v2.0 (Certicom, May 2009) §2.3.3/§2.3.4 point encoding and decoding for SM2. Identical in
//! shape to [`crate::p256_sec1`] -- see that module's docs for the full reasoning -- with SM2's
//! own field type and domain constants substituted. `draft-shen-sm2-ecdsa-02` itself does not
//! specify a byte-level point encoding (it defines the "point compression" notation `y~P` but not
//! the wire format), so this reuses SEC 1's near-universal encoding, the same convention every
//! other curve in this crate follows and the one real-world SM2 implementations interoperate with.
//!
//! # Encodings
//!
//! Field elements are encoded big-endian, fixed-width (SEC 1 §2.3.5/§2.3.7 for `GF(p)`) -- the
//! opposite byte order from this crate's internal little-endian `u64` limbs, so
//! [`limbs_from_be_bytes`] and [`be_bytes_from_limbs`] convert at the boundary.
//!
...
//! directly rather than a [`Sm2JacobianPoint`], not a `[u8; 1]` "point at infinity" case: SEC 1
//! §2.3.3 case 1 encodes `∅` as a single `0x00` octet, but an ECDSA-shaped public key -- the only
//! caller -- can never legitimately be `∅`, so pushing the infinity check to the type (a caller
//! must have already gotten `Some(x, y)` out of [`Sm2JacobianPoint::to_affine`]) makes encoding
//! infinity a compile error instead of a runtime one.
//!
//! [`decode`] is SEC 1 §2.3.4 cases 2 and 3 (again, no case 1, for the same reason). It performs
//! partial public-key validation as part of decoding, not as a separate step: not-infinity holds
//! by construction (see above), `x`, `y` in `[0, p)` is [`limbs_less_than_p`], and on-the-curve is
//! explicit for the uncompressed case and automatic for the compressed case, since its `y` is
//! constructed as an actual square root of `x^3 - 3x + b`. The additional "full validation" step,
//! `nQ = ∅`, is not needed: this curve has cofactor `h = 1` (derived directly from the Hasse bound
//! using `p` and `n` from `draft-shen-sm2-ecdsa-02` Appendix D -- `|n - (p+1)| <= 2*sqrt(p)` holds
//! only for `h = 1`, verified in Python, not assumed by analogy to any other curve), so a point
//! that passes partial validation has order dividing the prime `n` and is not the identity, hence
//! has order exactly `n` -- `nQ = ∅` is implied, not a separate check.
//!
//! # Compressed decoding
//!
//! `p ≡ 3 (mod 4)` (verified directly in this module's tests), so a square root of a residue `α`
//! is `β = α^((p+1)/4) mod p` (`β² = α^((p+1)/2) = α^((p-1)/2) * α = α` when `α` is a residue, by
...
use crate::nat;
use crate::sm2::{P_LIMBS, Sm2FieldElement};
use crate::sm2_domain::B_LIMBS;
...
/// `y^2 == x^3 - 3x + b`.
fn is_on_curve(x: &Sm2FieldElement, y: &Sm2FieldElement) -> bool {
    let lhs = y.mul(y);
    let x_cubed = x.mul(x).mul(x);
    let three_x = x.add(x).add(x);
    let rhs = x_cubed.sub(&three_x).add(&Sm2FieldElement::from_limbs(B_LIMBS));
    lhs == rhs
}
...
fn pow_public(base: &Sm2FieldElement, exponent_limbs: &[u64; 4]) -> Sm2FieldElement {
    let mut result = Sm2FieldElement::ONE;
...
pub fn encode_uncompressed(x: &Sm2FieldElement, y: &Sm2FieldElement) -> [u8; 65] {
    let mut out = [0u8; 65];
    out[0] = 0x04;
...
pub fn encode_compressed(x: &Sm2FieldElement, y: &Sm2FieldElement) -> [u8; 33] {
    let mut out = [0u8; 33];
...
pub fn decode(bytes: &[u8]) -> Option<(Sm2FieldElement, Sm2FieldElement)> {
    match bytes.len() {
        33 => {
            ...
            let x = Sm2FieldElement::from_limbs(x_limbs);
            let x_cubed = x.mul(&x).mul(&x);
            let three_x = x.add(&x).add(&x);
            let alpha = x_cubed.sub(&three_x).add(&Sm2FieldElement::from_limbs(B_LIMBS));
        ...
        65 => {
            ...
            let x = Sm2FieldElement::from_limbs(x_limbs);
            let y = Sm2FieldElement::from_limbs(y_limbs);
            if !is_on_curve(&x, &y) {
                return None;
            }
            Some((x, y))
...
/// [`Sm2JacobianPoint`].
pub fn decode_point(bytes: &[u8]) -> Option<Sm2JacobianPoint> {
    let (x, y) = decode(bytes)?;
    Some(Sm2JacobianPoint::from_affine(x, y))
}
...
mod tests {
    ...
    fn limbs_less_than_p_boundary_cases() {
        ...
        let p_plus_2_pow_64 = [P_LIMBS[0], P_LIMBS[1] + 1, P_LIMBS[2], P_LIMBS[3]];
        assert!(!limbs_less_than_p(&p_plus_2_pow_64));
