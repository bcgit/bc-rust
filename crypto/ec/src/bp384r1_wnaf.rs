...
//! brainpoolP384r1 ECDSA verification. Identical algorithm to [`crate::p256_wnaf`] -- see that
//! module's docs for the full derivation and verification methodology -- with the limb count
//! swapped for brainpoolP384r1's (`WNAF_LEN = 385`, one more than `n`'s 384-bit length, same
//! margin as P-384's own `385 = 384 + 1`).
//!
//! `G`'s odd multiples are a compile-time affine table (`bp384r1_wnaf_table`, width 7) added by mixed
...
use crate::bp384r1_scalar::Bp384r1PublicScalar;
use crate::bp384r1_wnaf_table::{G_ODD_MULTIPLES_X, G_ODD_MULTIPLES_Y};
use crate::nat;
...
pub fn shamir_multiply(
    u: &Bp384r1PublicScalar,
    v: &Bp384r1PublicScalar,
    q: &Bp384r1JacobianPoint,
...
/// holds `(2i+1)*p`.
fn odd_multiples(p: &Bp384r1JacobianPoint) -> [Bp384r1JacobianPoint; ODD_MULTIPLE_COUNT] {
...
/// digit `0`.
fn lookup_signed(
    table: &[Bp384r1JacobianPoint; ODD_MULTIPLE_COUNT],
    digit: i8,
) -> Option<Bp384r1JacobianPoint> {
...
fn lookup_signed_g(digit: i8) -> Option<(Bp384r1FieldElement, Bp384r1FieldElement)> {
    if digit == 0 {
        return None;
    }
    let index = (digit.unsigned_abs() as usize - 1) / 2;
    let x = Bp384r1FieldElement::from_limbs(G_ODD_MULTIPLES_X[index]);
    let y = Bp384r1FieldElement::from_limbs(G_ODD_MULTIPLES_Y[index]);
