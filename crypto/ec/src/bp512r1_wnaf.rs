...
//! brainpoolP512r1 ECDSA verification. Identical algorithm to [`crate::p256_wnaf`] -- see that
//! module's docs for the full derivation and verification methodology -- with the limb count
//! swapped for brainpoolP512r1's (`WNAF_LEN = 513`, one more than `n`'s 512-bit length, same
//! margin as P-384's own `385 = 384 + 1`).
//!
//! `G`'s odd multiples are a compile-time affine table (`bp512r1_wnaf_table`, width 7) added by mixed
...
use crate::bp512r1_scalar::Bp512r1PublicScalar;
use crate::bp512r1_wnaf_table::{G_ODD_MULTIPLES_X, G_ODD_MULTIPLES_Y};
use crate::nat;
...
pub fn shamir_multiply(
    u: &Bp512r1PublicScalar,
    v: &Bp512r1PublicScalar,
    q: &Bp512r1JacobianPoint,
...
fn compute_wnaf(k_limbs: [u64; 8], width: usize) -> [i8; WNAF_LEN] {
    let mut digits = [0i8; WNAF_LEN];
    let mut k = k_limbs;
    let mut pos = 0;
    while k != [0, 0, 0, 0, 0, 0, 0, 0] {
        if k[0] & 1 == 1 {
            ...
            k = if digit >= 0 {
                let (new_k, borrow) = nat::sub(&k, &[digit as u64, 0, 0, 0, 0, 0, 0, 0]);
                debug_assert_eq!(borrow, 0);
                new_k
            } else {
                let (new_k, carry) = nat::add(&k, &[(-digit) as u64, 0, 0, 0, 0, 0, 0, 0]);
...
/// holds `(2i+1)*p`.
fn odd_multiples(p: &Bp512r1JacobianPoint) -> [Bp512r1JacobianPoint; ODD_MULTIPLE_COUNT] {
...
/// digit `0`.
fn lookup_signed(
    table: &[Bp512r1JacobianPoint; ODD_MULTIPLE_COUNT],
    digit: i8,
) -> Option<Bp512r1JacobianPoint> {
...
fn lookup_signed_g(digit: i8) -> Option<(Bp512r1FieldElement, Bp512r1FieldElement)> {
    if digit == 0 {
        return None;
    }
    let index = (digit.unsigned_abs() as usize - 1) / 2;
    let x = Bp512r1FieldElement::from_limbs(G_ODD_MULTIPLES_X[index]);
    let y = Bp512r1FieldElement::from_limbs(G_ODD_MULTIPLES_Y[index]);
