...
impl P521JacobianPoint {
    ...
    pub(crate) fn add_affine(
        ...
        let is_opposite_point = h_is_zero & !r_is_zero;
        let doubled = self.double();
        let other = Self { x: *x2, y: *y2, z: P521FieldElement::ONE };
    ...
    pub(crate) fn add_vartime_affine(&self, x2: &P521FieldElement, y2: &P521FieldElement) -> Self {
        if self.is_infinity().to_bool() {
...
mod tests {
    ...
    fn add_affine_agrees_with_add_on_every_case() {
        ...
        let z = P521FieldElement::from_limbs(G_Y_LIMBS);
        let z2 = z.mul(&z);
        let scaled_g = P521JacobianPoint { x: g.x.mul(&z2), y: g.y.mul(&z2.mul(&z)), z };
        let selves = [g, g.double(), g.negate(), P521JacobianPoint::INFINITY, scaled_g];
    ...
    fn add_vartime_affine_agrees_with_add_on_every_case() {
        ...
        let z2 = z.mul(&z);
        let scaled_g = P521JacobianPoint { x: g.x.mul(&z2), y: g.y.mul(&z2.mul(&z)), z };
        for a in [g, g.double(), g.negate(), P521JacobianPoint::INFINITY, scaled_g] {
