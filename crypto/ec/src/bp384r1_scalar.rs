...
impl Bp384r1PublicScalar {
    ...
    /// `self^-1 mod n` in **variable time**, as a [`Bp384r1ScalarField`] ready for the arithmetic
    ...
    /// and not on [`Bp384r1ScalarField`], whose `invert` is the constant-time one. See
    /// [`crate::inverse_vartime`].
    pub fn invert_vartime(&self) -> Bp384r1ScalarField {
        Bp384r1ScalarField::from_limbs(crate::inverse_vartime::mod_inverse(&self.0, &N_LIMBS))
    }
}
