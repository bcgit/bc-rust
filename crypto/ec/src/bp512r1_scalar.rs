...
impl Bp512r1PublicScalar {
    ...
    /// `self^-1 mod n` in **variable time**, as a [`Bp512r1ScalarField`] ready for the arithmetic
    ...
    /// and not on [`Bp512r1ScalarField`], whose `invert` is the constant-time one. See
    /// [`crate::inverse_vartime`].
    pub fn invert_vartime(&self) -> Bp512r1ScalarField {
        Bp512r1ScalarField::from_limbs(crate::inverse_vartime::mod_inverse(&self.0, &N_LIMBS))
    }
}
