...
//! 256 + 1`).
//!
//! `G`'s odd multiples are a compile-time affine table (`p521_wnaf_table`, width 7) added by mixed
...
use crate::p521_wnaf_table::{G_ODD_MULTIPLES_X, G_ODD_MULTIPLES_Y};
...
// the `WIDTH` passed to it (unchanged), so the extra entries are simply never indexed -- an
// accepted mutant.
...
fn compute_wnaf(k_limbs: [u64; 9], width: usize) -> [i8; WNAF_LEN] {
...
fn lookup_signed_g(digit: i8) -> Option<(P521FieldElement, P521FieldElement)> {
    if digit == 0 {
        return None;
    }
    let index = (digit.unsigned_abs() as usize - 1) / 2;
    let x = P521FieldElement::from_limbs(G_ODD_MULTIPLES_X[index]);
    let y = P521FieldElement::from_limbs(G_ODD_MULTIPLES_Y[index]);
