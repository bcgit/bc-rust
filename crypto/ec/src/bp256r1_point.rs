...
impl Bp256r1JacobianPoint {
    ...
    pub(crate) fn add_affine(
        &self,
        x2: &Bp256r1FieldElement,
        y2: &Bp256r1FieldElement,
        ...
        let is_opposite_point = h_is_zero & !r_is_zero;
        let doubled = self.double();
        let other = Self { x: *x2, y: *y2, z: Bp256r1FieldElement::ONE };
...
mod tests {
    ...
    fn add_affine_agrees_with_add_on_every_case() {
        ...
        let z = Bp256r1FieldElement::from_limbs(G_Y_LIMBS);
        let z2 = z.mul(&z);
        let scaled_g = Bp256r1JacobianPoint { x: g.x.mul(&z2), y: g.y.mul(&z2.mul(&z)), z };
        let selves = [g, g.double(), g.negate(), Bp256r1JacobianPoint::INFINITY, scaled_g];
    ...
    fn add_vartime_affine_agrees_with_add_on_every_case() {
        ...
        let z2 = z.mul(&z);
        let scaled_g = Bp256r1JacobianPoint { x: g.x.mul(&z2), y: g.y.mul(&z2.mul(&z)), z };
        for a in [g, g.double(), g.negate(), Bp256r1JacobianPoint::INFINITY, scaled_g] {
