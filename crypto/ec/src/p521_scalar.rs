...
impl P521PublicScalar {
    ...
    /// `self^-1 mod n` in **variable time**, as a [`P521ScalarField`] ready for the arithmetic
    ...
    /// and not on [`P521ScalarField`], whose `invert` is the constant-time one. See
    /// [`crate::inverse_vartime`].
    pub fn invert_vartime(&self) -> P521ScalarField {
        P521ScalarField::from_limbs(crate::inverse_vartime::mod_inverse(&self.0, &N_LIMBS))
    }
}
