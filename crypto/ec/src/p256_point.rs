...
//! covering every exceptional case above plus the ordinary case.
...
impl P256JacobianPoint {
    ...
    fn generic_add(&self, other: &Self) -> (Self, P256FieldElement, P256FieldElement) {
    ...
    pub(crate) fn add_affine(
        ...
        let is_opposite_point = h_is_zero & !r_is_zero;
        let doubled = self.double();
        let other = Self { x: *x2, y: *y2, z: P256FieldElement::ONE };
    ...
    pub(crate) fn add_vartime_affine(&self, x2: &P256FieldElement, y2: &P256FieldElement) -> Self {
        if self.is_infinity().to_bool() {
...
fn select_point(
    ...
    P256JacobianPoint {
        x: P256FieldElement::from_internal_limbs(select_limbs(
            cond,
            &a.x.internal_limbs(),
            &b.x.internal_limbs(),
        )),
        y: P256FieldElement::from_internal_limbs(select_limbs(
            cond,
            &a.y.internal_limbs(),
            &b.y.internal_limbs(),
        )),
        z: P256FieldElement::from_internal_limbs(select_limbs(
            cond,
            &a.z.internal_limbs(),
            &b.z.internal_limbs(),
        )),
...
mod tests {
    ...
    fn add_affine_agrees_with_add_on_every_case() {
        ...
        let z = P256FieldElement::from_limbs(G_Y_LIMBS);
        let z2 = z.mul(&z);
        let scaled_g = P256JacobianPoint { x: g.x.mul(&z2), y: g.y.mul(&z2.mul(&z)), z };
        let selves = [g, g.double(), g.negate(), P256JacobianPoint::INFINITY, scaled_g];
    ...
    fn add_vartime_affine_agrees_with_add_on_every_case() {
        ...
        let z2 = z.mul(&z);
        let scaled_g = P256JacobianPoint { x: g.x.mul(&z2), y: g.y.mul(&z2.mul(&z)), z };
        for a in [g, g.double(), g.negate(), P256JacobianPoint::INFINITY, scaled_g] {
