...
impl<PARAMS: Sha512Family> SHA512Internal<PARAMS> {
    ...
    fn finalize(mut self, partial_byte: u8, num_partial_bits: usize, output: &mut [u8]) -> usize {
        ...
        // FIPS 180-4 s. 5.1.2: final message byte = [partial bits, MSB-first] [1] [0...].
        ...
        // FIPS 180-4 s. 5.1.2: append the 128-bit big-endian message length l in bits. byte_count is a
        // byte counter, so the high 64 bits are byte_count >> 61 and the low 64 bits are
        // (byte_count << 3) | num_partial_bits (the low three bits of byte_count << 3 are zero).
