...
//! * **A short SHA-512/t truncation is a weak hash.** [`SHA512t`] accepts any `T` FIPS 180-4
//!   s. 5.3.6 defines a hash for, and the smaller `T` is, the less collision resistance it
//!   offers: SHA-512/8 has a one-byte digest, and every `T` below 224 reports
//!   `SecurityStrength::None`. Pick `T` for the security level you need, not for the digest size
//!   you would like.
...
/// hash value, truncated to `T` bits.
///
/// This documentation explains how to instantiate the [`SHA512t`] struct from outside the library
/// with a truncation length other than the 224 and 256 specified in FIPS 180-4.
///
/// # Which `T` are accepted
///
/// FIPS 180-4 s. 5.3.6 defines SHA-512/t for every "positive integer without a leading zero such
/// that t < 512, and t is not 384", and then names two members of that family, SHA-512/224 and
...
/// // FIPS 180-4 s. 5.3.6: "t is not 384" -- use SHA384, which has its own IV.
/// let _ = SHA512t::<384>::new();
/// ```
///
/// ```compile_fail
/// use bouncycastle_sha2::SHA512t;
/// // Not a whole number of bytes: this crate requires t to be a multiple of 8.
/// let _ = SHA512t::<100>::new();
/// ```
///
/// ```compile_fail
/// use bouncycastle_sha2::SHA512t;
/// // FIPS 180-4 s. 5.3.6: "t < 512" -- use SHA512 for the untruncated hash.
/// let _ = SHA512t::<512>::new();
/// ```
///
/// # The initial hash value
///
/// What makes SHA-512/t a different hash from "SHA-512, keep the first t bits" is its initial
/// hash value H(0): the eight 64-bit words the compression function starts from. FIPS 180-4
/// s. 5.3.6 requires this of the family -- "Each hash function requires a distinct initial hash
/// value" -- so where s. 5.3.5 fixes H(0) for SHA-512, s. 5.3.6's IV Generation Function derives
/// a fresh one for each t by hashing the ASCII string "SHA-512/t" with a modified SHA-512. Because
/// the starting state differs, a SHA-512/t digest is not a prefix of the SHA-512 digest of the
/// same message, and digests for different t are unrelated to one another rather than prefixes of
/// a common value. H(0) is not a parameter the caller supplies: [`SHA512tParams`] computes it at
/// compile time from `T`, so a new `T` costs nothing at runtime and needs no table, and for
/// t = 224 and t = 256 the result is pinned by `tests/sha512t_h0_tests.rs` against the words
/// FIPS 180-4 prints in s. 5.3.6.1 and s. 5.3.6.2.
///
/// # Example
///
/// ```
...
///
/// // A custom truncation: 96 bits, so a 12-byte digest.
/// let digest = SHA512t::<96>::new().hash(b"abc");
/// assert_eq!(digest.len(), 12);
/// assert_eq!(<SHA512t<96> as Algorithm>::ALG_NAME, "SHA512/96");
/// ```
///
/// Only [`SHA512_224`] and [`SHA512_256`] have an assigned [`AlgorithmOID`] and a `HashFactory`
/// entry; every other `T` is reachable only by naming it in code, as above.
...
impl<const T: usize> HashAlgParams for SHA512tParams<T> {
    ...
    /// multiple of 8 (checked by `check_t` in `sha512.rs`), so this is exact.
