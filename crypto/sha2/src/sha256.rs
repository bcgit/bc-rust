...
impl<PARAMS: SHA2Params> Sha256State<PARAMS> {
    ...
    fn compress(&mut self, blocks: &[[u8; 64]]) {
        if crate::asm::sha256::try_compress(&mut self.h, blocks) {
            return;
        }

        self.compress_scalar(blocks);
    }

    /// Portable compression function, FIPS 180-4 section 6.2.2.
    pub(crate) fn compress_scalar(&mut self, blocks: &[[u8; 64]]) {
        let mut x = [0u32; 64];
